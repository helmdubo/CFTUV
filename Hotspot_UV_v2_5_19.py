...
def classify_boundary_loops_3d(loops, patch_faces):
    ...
    # Центроид non-convex полигонов (P, L, U) часто за пределами фигуры →
    # ломает nesting. Вместо этого: nudge midpoint рёбер внутрь, берём первый
    # что реально внутри полигона.
    def interior_point(poly):
        ...
        # Сортируем рёбра по длине (longest first — выше шанс попасть внутрь)
        edges_by_len = []
        for i in range(n):
            j = (i + 1) % n
        ...
        for len2, i in edges_by_len:
            ...
            # Inward normal: CCW (sa>0) → left = (-dy, dx), CW → right = (dy, -dx)
            if sa >= 0:
        ...
        # Fallback: центроид (лучше чем ничего)
        return (sum(p[0] for p in poly) / n,
...
def analyze_all_patches(bm, base_faces):
    ...
    for patch_faces in patches:
        ...
        # Сохраняем координаты loop вершин — BMVert references умрут при mode switch
        ...
        # Сериализуем faces для debug mesh (BMFaces умрут при mode switch)
        # Триангулируем + смещаем вдоль нормали патча чтобы cover поверх оригинала
        vert_map = {}
        mesh_verts = []
        mesh_faces = []
        offset_dir = normal * 0.002  # 2mm offset
        for f in patch_faces:
            face_indices = []
            for v in f.verts:
                if v.index not in vert_map:
                    vert_map[v.index] = len(mesh_verts)
                    mesh_verts.append(v.co.copy() + offset_dir)
                face_indices.append(vert_map[v.index])
            # Триангуляция: quad → 2 tri, ngon → fan от первого vert
...
def _clear_gp_debug(source_obj):
    ...
    for mat in list(bpy.data.materials):
        if mat.name.startswith("CFTUV_P") and mat.users == 0:
...
def create_debug_visualization(patch_results, source_obj):
    """Создаёт GP strokes для визуализации анализа + mesh backup для patch fills."""
    ...
    # GP Patch_ID fill layer — per-patch materials + bridged contours
    patch_layer_name = "Patch_ID"
    if patch_layer_name in gp_data.layers:
        layer = gp_data.layers[patch_layer_name]
        layer.clear()
    else:
        layer = gp_data.layers.new(patch_layer_name, set_active=False)
    if not layer.frames:
        patch_frame = layer.frames.new(0)
    else:
        patch_frame = layer.frames[0]
    ...
    for pi, patch in enumerate(patch_results):
        hue = (pi * golden_ratio) % 1.0
        if patch['type'] == 'WALL':
            sat, val, alpha = 0.8, 0.9, 0.85
        else:
            sat, val, alpha = 0.5, 0.6, 0.7
        r, g, b = colorsys.hsv_to_rgb(hue, sat, val)
    ...
    for pi, patch in enumerate(patch_results):
        ...
        # Patch_ID fill — per mesh-face GP strokes (каждый face = convex tri/quad)
        # Гарантирует корректный fill на concave патчах и автоматические holes
...
class HOTSPOTUV_OT_DebugAnalysis(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Detailed console log per patch
        print("=" * 60)
        print("CFTUV Debug Analysis")
        print("=" * 60)
        for pi, p in enumerate(patch_results):
            n_faces = len(p['faces'])
            roles = [s['frame_role'] for s in p['all_segments']]
            role_str = " ".join(roles)
            print(f"  Patch {pi}: {p['type']} | {n_faces}f | "
                  f"loops:{len(p['loops'])} | segs:[{role_str}] | "
                  f"seed_t·UP={p['seed_t'].dot(WORLD_UP):.3f} seed_b·UP={p['seed_b'].dot(WORLD_UP):.3f}")
        print("=" * 60)
        ...
        self.report({"INFO"}, 
            f"Patches: {total_patches} (W:{walls} F:{floors} 1f:{singles}) | "
            f"Seg: {total_segs} H:{total_h} V:{total_v} Free:{total_free} | Holes: {total_holes}")
        return {"FINISHED"}
