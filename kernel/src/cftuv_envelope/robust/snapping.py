...
def admissible_scales(window: GridWindowV1) -> tuple[int, ...]:
    """Все степени двойки, шаг которых лежит внутри окна, по возрастанию.

    Возрастание масштаба — это убывание шага, то есть список идёт от самого
    КРУПНОГО допустимого шага к самому мелкому. Порядок здесь канонический и
    ничего не выбирает: закон выбора живёт в `source_grid` и объявляет, с
    какого конца этого списка идти. Разделено намеренно — множество кандидатов
    задаётся окном и от закона не зависит, иначе «внутри окна» означало бы
    разное при разных законах.

    `window.grid` — самый мелкий допустимый шаг, поэтому список кончается
    именно им; более крупные получаются последовательным удвоением шага, пока
    он не выйдет за верхнюю границу. Нижняя при этом не проверяется отдельно:
    удвоение шага только удаляет его от нижней границы.
    """
    ...
    while Fraction(1, scale) <= window.upper_bound:
        scales.append(scale)
        if scale == 1:
            break
        scale //= 2
    return tuple(reversed(scales))
