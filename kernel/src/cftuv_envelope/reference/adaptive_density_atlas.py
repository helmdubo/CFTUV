...
# Объявленный кап точной работы ПОДГОТОВКИ власти веера — всего, что считается
# ДО поиска D*. Категория NUMERIC/WORK budget: превышение не меняет ни одного
# ответа, оно превращает бесконечный счёт в именованный отказ.
#
# Зачем отдельный литерал. Кап поиска D* заводится уже внутри поиска, и всё,
# что посчитано раньше, не оплачено ничем. А раньше считаются две величины,
# не ограниченные ничем, кроме входа:
#   * legacy-развёртка ordinal-окна перебирает ceil(1/sqrt(w)) знаменателей и
#     растёт без границы при сужении окна w;
#   * позитивный свидетель termination-box проверяет 2^k угловых
#     последовательностей на каждое уточнение и растёт экспоненциально по
#     числу ordinal-окон k.
# Пока они не оплачены, домен уходит в счёт без исхода — это хуже отказа
# (AGENTS.md п.4: тихих путей нет).
#
# ИЗМЕРЕНО (обе величины — расход одной подготовки, `spent`):
#   * полный зелёный корпус ядра, 2767 тестов, 351 подготовка: максимум 32;
#   * полевые слепки walls.012 и wall_2.001 на плотностях 0…4: максимум 36
#     (walls.012, density 3), у самой глубокой зелёной власти поля — 8
#     (см. `test_fan_preparation_work_stays_far_below_its_cap`).
#
# Литерал выбран НЕ по марже, а по ЦЕНЕ ПОТОЛКА, и это единственный честный
# способ его назвать: 65 536 знаменателей точной развёртки стоят около 0.3 s
# (измерено: 44 722 знаменателя — 0.19 s), то есть сам потолок ещё остаётся в
# бюджете подготовки домена, а всё, что дороже, уже не подготовка, а зависание.
# Запас над измеренным худшим случаем при этом ~1800x: кап обязан ловить
# патологию входа, а не подрезать здоровый домен, который просто шире
# измеренных. По закону 2^k он оставляет termination-box полные уточнения при
# девяти ordinal-окнах, а legacy-развёртке — окно шириной до 2.3e-10.
# Контроль цены снаружи капа: окно 5e-14 без него считается 53.95 s, с ним
# отказывает именем за 0.0016 s.
DENSITY_FAN_PREPARATION_WORK_CAP = 1 << 16


def preparation_budget(exhausted, cap: int | None = None):
    """Свежий счётчик точной работы подготовки одной власти.

    Механизм отказа тот же, что у поиска D*: подготовка и поиск оплачивают
    одну и ту же власть, и полю не нужен второй словарь исходов, чтобы понять,
    что власть не построена. Кап у них разный, и по нему исход различим — в
    тексте отказа стоит `cap=`.
    """

    return DensityExactWorkBudget(
        exhausted,
        DENSITY_FAN_PREPARATION_WORK_CAP if cap is None else cap,
    )
