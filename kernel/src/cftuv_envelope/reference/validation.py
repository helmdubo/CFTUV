...
from .planar_types import CertifiedPredicateUndecidable, exact_sign
...
def _interval_contains_oriented_support_delta(
    ...
    incoming_length = sp.sqrt(sum(item * item for item in incoming))
    outgoing_length = sp.sqrt(sum(item * item for item in outgoing))
    if exact_sign(incoming_length) <= 0 or exact_sign(outgoing_length) <= 0:
        return False
    incoming_unit = tuple(item / incoming_length for item in incoming)
    outgoing_unit = tuple(item / outgoing_length for item in outgoing)
    cross_value = sp.factor(
...
def validate_reference_geometry_certificates(
    ...
    if domain is None or len(frames) != 1:
        return ()
    frame = frames[0]
    ...
    for relation in sorted(
        ...
        certificate = certificate_by_id.get(relation.reflex_angle_certificate_id)
        if certificate is None or not isinstance(
            certificate.measure_payload, CertifiedReflexAngleMeasureV1
        ):
            continue
        incoming_payload = sector.incoming_support_ref.direction_payload
        outgoing_payload = sector.outgoing_support_ref.direction_payload
        if not isinstance(
            incoming_payload, CertifiedPlanarSupportDirectionV1
        ) or not isinstance(outgoing_payload, CertifiedPlanarSupportDirectionV1):
            continue
        incoming = (
            _scalar(incoming_payload.direction_in_domain.x),
            _scalar(incoming_payload.direction_in_domain.y),
        )
        outgoing = (
            _scalar(outgoing_payload.direction_in_domain.x),
            _scalar(outgoing_payload.direction_in_domain.y),
        )
        try:
