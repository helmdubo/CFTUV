...
def exact_overlay_view(builder, overlay):
    ...
    # проверено по всем шести местам, где вид создаётся, — поэтому «поколение»
    ...
    return ExactCandidateViewV1(
        ...
        budget,
        # Память мест — та же самая, что у рантаймового вида: символьный слой
        # спрашивает места ТЕХ ЖЕ прямых в ТОТ ЖЕ момент, и второй словарь
        # рядом означал бы, что одна и та же чистая функция считается дважды.
        getattr(builder, "_position_memo", None),
    )
