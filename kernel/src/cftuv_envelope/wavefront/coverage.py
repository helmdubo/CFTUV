...
Это не приближение и не ещё одна модель. Нормаль в `SupportLineV1` имеет длину
ровно `sqrt(q)`, поэтому `(a*p - c)/sqrt(q) = dist(p, line)` ТОЧНО, а время
прихода фронта ребра в точку своей грани равно этому расстоянию. То есть
неравенство выше — буквально «время прихода не больше alpha», и никакой второй
геометрии для него не понадобилось.
