...
def _line_of(face: FaceV1) -> SupportLineV1:
    ...
    Скорость при этом берётся из самой грани, а не из длины ребра: концы задают
    нормаль, но не `q`, и восстановление `q = |d|^2` было бы верно ровно для
    единичной скорости.
    """

    return SupportLineV1.with_speed(
        face.source_start, face.source_end, face.speed_squared
    )
