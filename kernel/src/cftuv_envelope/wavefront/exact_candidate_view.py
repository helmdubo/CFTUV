...
class SpanStateMemoV1:
    ...
    ЧТО ИЗМЕРЕНО. `exact_overlay_view.span_state` пересобиралась на КАЖДЫЙ
    ...
    ЗНАК точной величины. Счёт конструкций `CandidateSpanStateV1`
    (`artifacts/exact_identity_intern/span_state_rebuilds.py`): wall 2.001
    density 0 — 30 634 пересборки против 2 401 с памятью, walls.012 density 0 —
    2 639 против 383, при тех же исходах и тех же единицах бюджета. Один и тот
    же пролёт пересобирался по десятку раз подряд, и каждая пересборка давала
    побитово тот же объект.
    ...
    переживает ни одной его мутации (все шесть мест, где вид строится, читают
