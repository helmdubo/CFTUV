...
class BridgeOutcome(str, Enum):
    ...
    # ВОВСЕ: якорь уехал с петли. Якорь угла — вершина домена по построению, но
    # привязка двигает вершины. Сопоставить веер «ближайшей» вершине значило бы
    # придумать вершину. Случай «якорь сел в узел, но не единственным» сюда НЕ
    # входит — узел-то принадлежит петле; его называет
    # `LATTICE_SNAP_BREAKS_A_VERTEX_FAN` ниже.
