...
(в) Вершина не в стоп-листе якорей. Стоп-лист приходит подсчётом ссылок по
    ...
    Строит стоп-лист `conveyor._domain_anchor_points`, а не этот модуль, и это
    не разделение ради разделения: якоря живут в `GeometryContext`, а перевод
    их в дроби делает `conveyor.exact_rational`. Импортировать её сюда значило
    бы завести цикл — конвейер уже импортирует растворение. Закон остаётся
    чистой геометрией, а сбор ссылок — там, где ссылки есть.
