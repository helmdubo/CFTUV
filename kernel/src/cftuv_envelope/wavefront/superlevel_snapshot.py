...
def _required_physical_edge_keys(builder, events) -> set[tuple]:
    ...
    `events` в подписи ТЕПЕРЬ РАБОТАЕТ, но ровно в одном месте и не радиусом.
    Пакет, у которого не осталось ни одного ЖИВОГО кандидата, не требует ни
    одного вхождения: транзакция возвращается до символьного слоя, а два её
    единственных до-выходных потребителя — запись неподдержанного вида события
    и запись двойного владения — читают только структурные факты (`ident`,
    `alive`, `next_edge`, ключи рёбер) и ни одного точного места. Это не
    сужение множества по расстоянию, а утверждение о ДОСТИЖИМОСТИ: спросить
    место здесь просто некому. Измерено на wall 2.001: 59 пакетов из 77
    бросаются до символьного слоя, и 76 гидратаций из 2 415 платились в них ни
    за чем.

    Для пакета, у которого живой кандидат ЕСТЬ, множество остаётся ПОЛНЫМ, и
    это по-прежнему цена правильности: замыкание зависимостей такого пакета
    измерено и равно всему живому фронту — тождество порта требует обоих концов
    обоих его плеч (`_consumed_by_locus`, `_leaf_bindings`, `_initial_vertices`
    читают вхождения КАЖДОЙ живой вершины). Поэтому «считать по требованию»
    и «считать всё» здесь совпадают по МНОЖЕСТВУ; расходятся они по ЧИСЛУ
    ВЫЧИСЛЕНИЙ, и это снимает память мест (`PositionMemoV1`), а не радиус.
    """
