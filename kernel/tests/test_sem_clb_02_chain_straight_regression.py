...
# ПЕРЕСЪЁМКА 2026-08-08, «НЕ ЭТИМ ЧИСЛОМ», карточка LAZY-FROZEN-HYDRATION.
# Четыре числа ПРОХОДА упали в 3.5 раза, два числа СВОЙСТВА не сдвинулись, и
# ответ не сдвинулся тоже. Причина названа и измерена: точное место вершины
# больше не пересчитывается на каждый вопрос — `position` отвечает памятью
# superlevel'а (`exact_candidate_view.PositionMemoV1`), ключ которой есть
# РОВНО аргументы формулы (две несущие прямые, закон скольжения, `t`).
# Пересчёт вернул бы те же коэффициенты, поэтому упало ЧИСЛО ВЫЧИСЛЕНИЙ, а не
# их результат.
#
# Почему это законно двигать, а дайджест — нет. Собственный докстринг ниже
# говорит, что здесь заморожена «the factual cost of the query-oriented
# implementation», то есть ЦЕНА. Снять цену плотной гидратации — предмет
# карточки; если бы это число не двигалось, карточка ничего бы не сделала.
# Свидетельства того, что двигается именно цена:
#   * ОТВЕТ проверяется ВЫШЕ по тому же тесту и прошёл: терминал конвейера,
#     терминал скелета и тождество замороженного простого базиса;
#   * `incomplete_picks`/`incomplete_supports` — те самые два СВОЙСТВА — как
#     были нулями, так и остались (pytest их и не показал в расхождениях);
#   * структурное отношение сохранилось до единицы: `coordinate_divisions`
#     равно ровно 2*`event_calls` и до пересъёмки (44 814 = 2*22 407), и
#     после (12 786 = 2*6 393) — упала кратность вызовов, а не закон;
#   * побитовое равенство ответа ленивого и плотного режимов проверяется
#     прогоном, а не рассуждением: `test_lazy_hydration_shadow.py` гоняет оба
#     режима на корпусе 63, на корпусе весов и стен, на длинных цепочках
#     смертей и на всех четырёх полевых фикстурах, включая ЭТУ.
# Было -> стало: event_calls 22 407 -> 6 393; coordinate_divisions
# 44 814 -> 12 786; pick_iterations 78 832 -> 21 908; support_checks
# 138 678 -> 38 548.
FROZEN_PATCH006_PASS = {
    "event_calls": 6393,
    "coordinate_divisions": 12786,
    "pick_iterations": 21908,
    "support_checks": 38548,
    # НОЛЬ ОТКАТОВ — это свойство, а не цена: базис полон, легаси-деление не
    # срабатывает ни разу. Пересъёмка ленивой гидратации его НЕ тронула.
    "incomplete_picks": 0,
