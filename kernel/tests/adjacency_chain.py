...
измерено полностью и чинит обе известные поломки порядка — полевой патч `bf6` и
весь остаток креста.

ПРЕПЯТСТВИЕ К ПОСТАНОВКЕ БЫЛО И СНЯТО, и это записано здесь, потому что иначе
следующий срез не поймёт, почему кандидат ждал. На трёх фигурах корпуса
частичного источника кандидат превращал верный ГРОМКИЙ ОТКАЗ в число, которое
противоречило независимому митрованному эталону (`ell_12_source_edge_4` при
alpha 8: 120 против 96). Корень лежал не в сборщике, а в `skeleton.py`: фронт
гасил коллинеарную неподвижную СТЕНУ, с которой его отрезок перекрывался в
единственной точке, и скачком расширялся вдвое. Стена разобрана, три фигуры
отказывают до всякого числа, противоречащих строк ноль. Числа перехода —
`test_wavefront_partial_source.py`; здесь — сама реализация, чтобы измерение
было воспроизводимым, а не пересказанным.
