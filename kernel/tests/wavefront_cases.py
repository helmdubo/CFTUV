...
def cross(
    *, wide: int = 4, tall: int = 4, right: int = 18, top: int = 20
) -> PolygonV1:
    ...
    В `named_corpus()` крест НЕ входит, и причина измерена, а не вкусовая: на
    нём сборщик граней ломается двумя разными способами, и оба назывались бы
    падением двух чужих тестов, а не находкой.

    - `wide == tall` (квадратный блок): очередь отказывает
      `WAVEFRONT_LEFT_UNRESOLVED`, потому что блок схлопывается разом;
    - `wide != tall`: `build_faces` отказывает
      `FACE_AREA_DOES_NOT_REPRODUCE_POLYGON` — сумма площадей граней меньше
      площади фигуры, и величина потери лежит числом в `detail`.

    Второй случай раньше был ТИХИМ (`EXACT` при нарушенной границе 1); теперь
    он громкий, но не починенный: сборщику нечем собрать правильную грань,
    вершина `(4 + wide/2, 4 + tall/2)` среди узлов скелета отсутствует. Пока
    это так, крест в `named_corpus()` не входит.

    Свидетельство и числа — `test_wavefront_faces.py` (сетка 3..9, 42 строки)
    и `test_wavefront_mitered_standard.py`.
    """
