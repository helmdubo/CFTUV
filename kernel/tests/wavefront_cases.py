...
def cross(
    *, wide: int = 4, tall: int = 4, right: int = 18, top: int = 20
    ...
    - `wide != tall`: `build_faces` возвращает `EXACT`, но собственная
      объявленная граница 1 у него при этом НАРУШЕНА — сумма площадей граней
      меньше площади фигуры на 1/2 либо 2 (удвоенный дефект −1 либо −4).

    Свидетельство и числа — `test_wavefront_mitered_standard.py`.
    """
