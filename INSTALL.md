...
Двойной клик по `tools\install_to_blender.bat`. Он сам находит Blender в
конфиге пользователя, кладёт аддон в `scripts\addons`, ядро в
`scripts\modules`, вычищает `__pycache__` и **сверяет отпечатки** копий с
репозиторием.

Заканчивается он не словом «готово», а сравнением хешей — потому что однажды
правка считалась установленной, не будучи ею: копию взяли из другой ветки,
полевой прогон показал прежнее поведение, и день ушёл на поиск
несуществующего бага.

```bat
tools\install_to_blender.bat                    :: новейшая найденная версия
tools\install_to_blender.bat -BlenderVersion 4.3
tools\install_to_blender.bat -WhatIf            :: показать пути и выйти
```

Зависимости (`sympy`) он **не** ставит — это разовое действие, см. шаг 2 ниже.

Ядро кладётся в `scripts\modules`, а не в `site-packages`: первое не требует
администратора и **перекрывает** устаревшую копию во втором, если она там
осталась с прежних установок.

Дальше — ручной путь, если батник почему-то не подходит.
