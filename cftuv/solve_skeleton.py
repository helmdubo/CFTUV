...
def _collect_runtime_shape_patch_ids(
    derived_topology: _PatchGraphDerivedTopology,
    quilt_patch_ids,
) -> frozenset[int]:
    if not quilt_patch_ids:
        return frozenset()
    runtime_shape_patch_ids = set()
    patch_shape_classes = getattr(derived_topology, "patch_shape_classes", {})
    for patch_id in quilt_patch_ids:
        if patch_shape_classes.get(patch_id, PatchShapeClass.MIX) != PatchShapeClass.MIX:
            runtime_shape_patch_ids.add(patch_id)
    band_spine_data = getattr(derived_topology, "band_spine_data", {})
    runtime_shape_patch_ids.update(
        patch_id for patch_id in quilt_patch_ids if patch_id in band_spine_data
    )
    straighten_chain_refs = getattr(derived_topology, "straighten_chain_refs", frozenset())
    runtime_shape_patch_ids.update(
        chain_ref[0] for chain_ref in straighten_chain_refs if chain_ref[0] in quilt_patch_ids
    )
    return frozenset(runtime_shape_patch_ids)
...
def _report_max_residual(report: SkeletonSolveReport) -> float:
    residuals = [
        float(residual)
        for residual in (report.residual_u, report.residual_v)
...
def apply_skeleton_solve(quilt: Any, diagnostics: Any = None) -> SkeletonSolveReport:
    ...
    runtime_shape_patch_ids = _collect_runtime_shape_patch_ids(
        quilt.derived_topology,
        quilt_patch_ids,
    )
    solve_patch_ids = quilt_patch_ids - runtime_shape_patch_ids
    ...
    if not solve_patch_ids:
        return SkeletonSolveReport(
            applied=False,
            quilt_scaffold=quilt.quilt_scaffold,
            notes=("skeleton_skip_runtime_shape_quilt",),
        )
    ...
    fixed_component_samples_u = _collect_component_coord_samples(
        quilt.graph,
        quilt.quilt_scaffold,
        skeleton_graphs,
        FrameAxisKind.COLUMN,
        allowed_patch_ids=runtime_shape_patch_ids,
    )
    fixed_component_samples_v = _collect_component_coord_samples(
        quilt.graph,
        quilt.quilt_scaffold,
        skeleton_graphs,
        FrameAxisKind.ROW,
    ...
    solved_u = _solve_axis(
        ...
        fixed_component_samples=fixed_component_samples_u,
    )
    solved_v = _solve_axis(
        ...
        fixed_component_samples=fixed_component_samples_v,
    )
    ...
    rebuilt_quilt_scaffold = _rebuild_quilt_scaffold(
        ...
        junctions_by_vert_index,
        quilt.final_scale,
        allowed_patch_ids=solve_patch_ids,
    )
    ...
    if runtime_shape_patch_ids:
        notes.append("skeleton_runtime_shape_passthrough")
...
def apply_skeleton_solve_to_scaffold_map(
    ...
    *,
    ...
    for quilt_scaffold in scaffold_map.quilts:
        ...
        is_pure_wall_side_quilt = _is_pure_wall_side_quilt(graph, quilt_scaffold)
        has_non_tree_closure = _quilt_has_non_tree_closure(quilt_plan)
        if has_non_tree_closure and not is_pure_wall_side_quilt:
            ...
            continue
        if has_non_tree_closure and is_pure_wall_side_quilt:
            trace_console(
                f"[CFTUV][Skeleton] Quilt {quilt_scaffold.quilt_index}: "
                f"allow non-tree closure for pure wall quilt"
            )
        ...
        if has_non_tree_closure and is_pure_wall_side_quilt:
            max_residual = _report_max_residual(report)
            if max_residual > SKELETON_MAX_CYCLE_RESIDUAL_APPLY:
                report = replace(
                    report,
                    applied=False,
                    quilt_scaffold=quilt_scaffold,
                    notes=("skeleton_skip_cycle_high_residual",),
                )
                trace_console(
                    f"[CFTUV][Skeleton] Quilt {quilt_scaffold.quilt_index}: "
                    f"revert cycle solve high_residual={max_residual:.6f}"
                )
        ...
        elif report.notes == ("skeleton_skip_runtime_shape_quilt",):
            trace_console(
                f"[CFTUV][Skeleton] Quilt {updated_quilt.quilt_index}: "
                f"skip runtime-shape quilt"
            )
        elif "skeleton_runtime_shape_passthrough" in report.notes:
            trace_console(
                f"[CFTUV][Skeleton] Quilt {updated_quilt.quilt_index}: "
                f"runtime-shape passthrough patches={sorted(runtime_shape_patch_ids)}"
            )
