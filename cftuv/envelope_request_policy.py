...
def envelope_angular_policy(kernel, density) -> EnvelopeAngularPolicyV1:
    ...
    ПОЧЕМУ ЗДЕСЬ ВСЁ ЕЩЁ `A`, хотя закон B («середины ячеек») уже есть в ядре.
    Карточка FAN-CELL-BOUNDARY-MIDPOINT требовала перевести плотности 0..4 на
    B. Перевод НЕ сделан, и это не забывчивость: закон B в его записанном виде
    ломает поле сильнее, чем чинит, — два числа, оба воспроизводятся
    `kernel/tests/test_density_midpoint_cell_law.py`:

    1. B переносит границу ячейки с ЧЁТНЫХ `q` на НЕЧЁТНЫЕ. Прямой угол
       (`u = 1/2`) у A сидел на границе при `q = 4, 6` (плотности 2 и 4);
       у B он сидит на границе при `q = 3, 5` (плотности 1 и 3), а d1 — это
       ДЕФОЛТ панели. Переключение обменяло бы одну пару плотностей на другую.
    2. У B подповорот законно превышает `pi/q` (полуполоса — названная цена
       принципала), а привязка направлений веера доказывает `<= pi/q` как
       обязательство сертификата. Поэтому на `building_002` (та самая стена с
       прямыми углами) плотности 2, 3 и 4 под B не компилируются вовсе:
       `REFERENCE_CERTIFIED_PREDICATE_UNDECIDABLE (BINDING_MONOTONE)`.

    Пока обе развилки не разрешены владельцем/принципалом, кнопка остаётся на
    законе A. Закон `None` (legacy `MIN_K_FOR_MAX_SUBTURN_V1`) не тронут.
    """
