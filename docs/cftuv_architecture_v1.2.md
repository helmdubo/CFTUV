...
## v1.2 - PatchGraph / Chain / Corner Update
...
8. `Corner` - часть анализа, а не solve
9. `segments` не являются runtime-сущностью модели
...
1. Flood fill faces в patch-и по seam / sharp / boundary rules
...
- использовать `PatchType`, `basis`, `loops`, `chains`, `corners`, `seam edges`
