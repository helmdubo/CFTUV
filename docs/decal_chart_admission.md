...
Статус: **DRAFT — требует подтверждения пользователя** (пред-условие C3
из `docs/CFTUV_decal_runtime_intrinsic_codex_workplan.md`). До
подтверждения admission-код не финализируется.
