...
4. **Multi-hop**: отображённая точка за следующей canonical
   transition в owner-space — авторитет уходит дальше по цепочке
   T7-P2.2(a); итерация ограничена R3 (`min(8, support_count)`),
   превышение = `ATLAS_TRANSITION_DESYNC`.
