...
3. **|declared tokens| >= 2 — tie-break ТОЛЬКО owner-space
   ...
   структурный провал O2 (отсутствует imported-кривая; чинится T7,
   не тайбрейком). Один класс (SEGMENT-цепочка) — выбор токена
   детерминирован интервалом: representative отображается изометрией
   transition в owner-chart, его проекция на T-ось попадает ровно в
   один станционный интервал → его владелец. Попадание проекции на
   станцию в пределах кванта означает, что face обязан был быть
   разрезан по T3 — диагностический стоп `ATLAS_TRANSITION_DESYNC`.
   Локальная дистанция как tie-break за пределами кванта запрещена.
