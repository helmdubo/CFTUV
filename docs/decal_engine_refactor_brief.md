...
- **S-CM.a — локальная семантика CornerModel** (до глобальной
  ...
  одна грань из трёх вершин. **+ S-DF2-lite** (внешний разбор,
  принят): аналитический labeled-оракул в приёмку —
  `DistanceWitness2D`-запросы, polygon-поле CornerModel,
  семплирующая сверка ИТОГОВОЙ материализованной геометрии с
  intended-полем (лекарство от Б5); без production-маршрутизации;
  предостережения (raster запрещён, pseudo-SDF после min/max,
  unsigned+side, поле не выбирает join) — в
  `docs/decal_wavefront_theory_note.md`.
- **S-WF0 — Surface Wavefront spike (ДИЗАЙН-ГЕЙТ перед R2/R3):**
  без production-маршрутизации; фикстуры/метрики (включая
  инвариантность к ретриангуляции) и исходы A/B/C — в
  `docs/decal_wavefront_theory_note.md`. Решает вопрос-гейт:
  rails на ГЛАДКОЙ кривизне — пути рёбер меша или характеристики
  интринсик-поля (обобщение RP1: рёбра ниже fold-порога на
  гладкой поверхности — артефакт тесселяции). Решение по исходу
  принимает ПОЛЬЗОВАТЕЛЬ; до него канон RP о rails не меняется.
