...
- После нажатия кнопки курсор переносится к экранной проекции центра bounding
  box исходного объекта. Это становится нулевой точкой drag и убирает зависимость
  диапазона от положения кнопки в N-панели. Если центр объекта вне экрана или не
  проецируется, используется центр окна 3D View.
