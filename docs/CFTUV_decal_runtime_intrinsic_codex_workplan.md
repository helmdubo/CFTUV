...
2. **RD-1 — «Скользящие рёбра» у краёв ленты** (скрины 3-4):
   ...
   **Результат RD-1:** триаж выполнен (`e0f730b`), стоп разрешён
   ...
   Идентификация пользовательского дефекта — ждёт подтверждения
   пользователя по аннотированному скрину
   `artifacts/decal_rd1_rf12_provenance_stop.png`; гипотеза
   ревьювера: повторяющиеся A10/alpha выемки production-пути,
   поглощаются R3. Policy НЕ меняется до подтверждения.
