...
1. **G3-impl** (решение принято, ветка (a) — см. секцию G3;
   мелкий диф): measured-first admission без resolution-порогов;
   proxy — ранний exit 0.5 rad; тесты по поправленному acceptance.
2. **C8.6** (materialization fail на большом меше, repeated vertex
   keys — НОВОЕ, третья волна; секция в C8-блоке).
3. **C8.3-cal** — данные сняты, design-stop ждёт решения ревьювера
   по таблице углов; порог не изменён. G1.1 завершён: imprint-root
   не подтвердился, needles локализованы в SMOOTH arrangement —
   это вход в решение C8.3-cal.
4. **G2** (border-крыло). G3+G2 закрывают полевую «юбку».
...
6. **Срез GL — Geometry Ledger** (§0c): консолидация представления
   атласной геометрии; 17 кейсов W1.4 — его чеклист; снятие
   saddle-xfail; затем **W2 — полный E-acceptance**.
7. **F1** (инкрементальность на M1) — гейт FP2 открывается полным
   E-acceptance.
