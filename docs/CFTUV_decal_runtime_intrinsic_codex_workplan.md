...
1. **C8.3-off** (решение принято, см. C8.3-cal): SMOOTH-константа
   -> 0, немедленное восстановление чистого MITER-выхода на
   полевой арке; однострочный диф + регрессия по таблице
   калибровки.
2. **C8.6** (materialization fail на большом меше, repeated vertex
   keys — третья волна; секция в C8-блоке): триаж по протоколу,
   fail блокирует большие выделения.
3. **C8.3-fix** (правильный SMOOTH pass-through): диагноз
   фрагментации wedge-fill, acceptance повторного включения — в
   секции C8.3-cal. G1.1 закрыт (imprint оправдан данными).
4. **G3-impl** (решение принято, ветка (a) — см. секцию G3; мелкий
   диф): measured-first admission без resolution-порогов; proxy —
   ранний exit 0.5 rad; тесты по поправленному acceptance.
5. **G2** (border-крыло). G3+G2 закрывают полевую «юбку».
6. **Merge GPU-ветки** `claude/decal-gpu-preview-f3` (F0-lite + F3
   готовы, 353 passed): rebase на стабилизированную основную ветку
   + ручной чек-лист acceptance F3 в Blender.
7. **Срез GL — Geometry Ledger** (§0c): консолидация представления
   атласной геометрии; 17 кейсов W1.4 — его чеклист; снятие
   saddle-xfail; затем **W2 — полный E-acceptance**.
8. **F1** (инкрементальность на M1) — гейт FP2 открывается полным
   E-acceptance.
