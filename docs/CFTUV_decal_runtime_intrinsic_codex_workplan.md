...
Иерархия выбора rail в вершине (ответ на ключевой вопрос):
0. seam/pChain-рёбра — НИКОГДА не rail, но ГРАНИЦА: rail
   терминируется/скользит вдоль них (даёт (a) бесплатно);
1. quad-loop продолжение: ребро, не делящее грань с входящим
   (edge-loop правило Blender) — детерминированно, покрывает
   quad-доминантные игровые меши, совпадает с ментальной моделью
   артиста;
2. straightest continuation (дискретная геодезика: минимум
   |сумма левых - сумма правых углов|) — fallback на
   нерегулярных вершинах;
3. явная разметка ребра артистом (override, edge-mark);
4. терминация: полюс/N-fan (нет продолжения -> CAP-политика),
   чужой chain (скольжение/стоп), alpha-reach.
...
готовом меше, а слияние НА ЭМИССИИ (materialization-time,
provenance-gated: same-owner обе стороны, не через классовые
границы, counted; намеренные сплиты G1.2/source-фолдов не
трогать).
