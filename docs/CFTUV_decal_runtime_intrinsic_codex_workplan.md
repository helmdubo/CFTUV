...
4. Регрессия на репро design-stop: saddle, width 0.25/0.5/1.0/2.0 —
   ...
   границы»; (ii) идентифицировать ребро station1->local без
   crop-provenance (ожидание: anchored owner-сепаратор T7-P3.11)
   -> кросс-чарт конформация локального crop-ребра к
   импортированной owner-кривой (14b), provenance-унификация всех
   каналов в boundary_corner_edges; (iii) per-side coverage на
   касательных участках (14c): сторона subedge-цепи crop-ребра —
   B0-факт, election читает его; span 1..5 chart14-side -> None,
   целевой sliver -> честный counted no-owner drop без
   материализации, T2-C зелёный; ассерт восстановленного
   инварианта «no-matter face не касается declared-span иначе как
   в станциях» на всех фикстурах; негатив: вернуть midpoint-
   election на касательном фикстурном участке -> T2-C обязан
   покраснеть; гейт 3.8 и негативы T7-P3.12 не деградируют;
