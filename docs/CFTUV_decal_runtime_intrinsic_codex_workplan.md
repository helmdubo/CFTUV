...
1. **§8b-фиксы T7-P2 ревью** (`docs/decal_atlas_design.md` §8b):
   два блокера (O1-UV оракул, boundary-owner fallback) + double-cover
   sweep + docs. Критерий: EP1 байт-в-байт. После — короткое
   подтверждение по diff и **полный E-acceptance**.
2. Параллельно (пользователь): ручная проверка rounded_wall drag в
   Blender на актуальном code commit.
3. **C8.1 -> C8.3 -> C8.2** (секция C8): полевые дефекты кривых
   поверхностей. НАМЕРЕННО меняет exact-путь; differential
   пересдаётся по каждому пункту. Только после E-acceptance.
4. **G1** (TRANCHE G): imprint source-рёбер — строго после C8.2
   (общий merge/key-слой).
