...
ВИЗУАЛЬНЫЙ КОНТРАКТ (полевой тест 3, три скрина пользователя —
определение готовности): V1 = imprint-пары соединены поперёк ленты
(G1.2); V2 = угол без V-клина + ноль сплитов на плоскости (C8.7,
C8.3-off/fix); V3 = сфера облегает (G3-impl + G2). Никакая работа,
не привязанная к V1-V3, не берётся до их закрытия (кроме C8.6 —
hard fail больших мешей).

1. **C8.3-off** — довести до сборки аддона, проверить на полевой
   арке (регрессия по таблице калибровки).
2. **C8.6** — materialization fail на большом меше (hard fail,
   блокирует большие выделения; секция в C8-блоке).
3. **G1.2** — попарные cross-band рёбра импринта (V1).
4. **C8.7** — V-клин на crest-углах + сплиты на плоскости (V2);
   вместе с C8.3-fix при пересечении корней.
5. **G3-impl** (решение принято, ветка (a)): measured-first без
   resolution-порогов; путь к V3.
6. **G2** (border-крыло) — завершает V3 (сфера облегает); для
   широких лент (полевой Seam Width 2.04) — рекомендация
   budget-knob в документации.
7. **Merge GPU-ветки** `claude/decal-gpu-preview-f3` — после V1-V3.
8. **Срез GL — Geometry Ledger** (§0c) -> **W2 — полный
   E-acceptance** — после V1-V3.
9. **F1** — гейт FP2 открывается E-acceptance.
