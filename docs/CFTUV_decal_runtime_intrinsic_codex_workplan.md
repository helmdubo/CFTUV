...
C8.3, C8.2, G1 (`723607d`), C8.5 (`bb6a9d7`). Актуальная очередь:

1. **Вторая волна полевых дефектов (stable path, видимо
   пользователю):** C8.3-cal — данные сняты, design-stop ждёт решения
   ревьювера; порог не изменён. G1.1 завершён: imprint-root не
   подтвердился, needles локализованы в том же SMOOTH arrangement.
2. **W4-fix — V-изометрия ширины intrinsic chart** (E-path,
   EP1-нейтрально; см. разрешение G3(b) в секции G3): v =
   перпендикулярная координата развёрнутой плоскости; оракул
   station-profile ассертом; re-sweep до монотонной сходимости.
3. **G3-решение** по валидным данным re-sweep (ожидание: исходная
   ветка (a) — measured-first admission).
