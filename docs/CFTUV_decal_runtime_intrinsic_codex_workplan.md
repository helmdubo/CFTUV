...
АКТУАЛЬНАЯ ОЧЕРЕДЬ (полевой тест 5; сборка `5dd6412f` =
R0+R1+LEGACY-CUT; нижеследующий старый список сохранён как история
результатов R0/R1):

0. **R1.2 — Терминальные rails (RR9, полевой тест 7):** торцы
   ленты скользят вдоль fold/border-rails торцевых вершин
   (иерархия pChain -> уникальный fold/border -> перпендикуляр);
   материализация читает rail-план, не пере-деривация. Фикстура
   RF16 + полевой скрин пользователя. ВПЕРЕДИ RD-1.
   **Результат R1.2:** IMPLEMENTED (`d0a8d6d`..`e0f730b`),
   ПРИНЯТО ревью. Materialization читает terminal choice из
   rail-плана (`RailTerminalUse` — единственный источник); ярус-2
   475 passed / 2 skipped / 2 atlas_frozen deselected; полевой
   building `RAIL_PLANAR:1c/3e`; чистый RR9(b) на wall_noise_top
   edge 60. Порог излома изменён 0.99 (~8.1 град.) -> cos(4
   град.) — узаконен как единственная семантическая константа
   fold-чувствительности (RP2a в оракуле); noise-top меши и
   half_sphere — ожидаемые strict ERROR до R3.
1. **R1.1 — Boundary-rails (RR8) + угол RM7** (скрины 1-2 теста
   5): поперечные seam-цепочки — принудительные rails/«берега»,
   торец ленты скользит вдоль них; угол на конце spine — простое
   русло БЕЗ митиринга (RM7). Фикстура RF11.
2. **RD-1 — «Скользящие рёбра» у краёв ленты** (скрины 3-4):
   триаж provenance продольных полосок внутри ленты (A10
   corner-вставка / offset-lift шов / RM5a-cap) -> удалить, если
   без семантической функции, либо доложить функцию ревьюверу до
   удаления. Фикстура RF12.
   **Результат RD-1:** триаж выполнен (`e0f730b`), стоп разрешён
   ревью: все рёбра несущие, удалений нет (см. RF12 в оракуле).
   Идентификация по скрину исполнителя пользователем СНЯТА
   («не могу понять что имелось ввиду») — RD-1 ЗАКРЫТ без смены
   policy; RF12 остаётся регрессией на приёмке R3. Если дефект
   проявится на rail-пути — новые полевые скрины со стрелками.
   **Полевое уточнение после RM9-fix:** пользователь идентифицировал
   отдельный более поздний класс — бирюзовые `CAP` pieces у торцов
   `sagging_wall`. Сам CAP закрывает owner surface и удаляться не может,
   но его внутреннее ребро и отдельная corner-UV развёртка не несут
   функции, когда существует ровно один копланарный SEGMENT-сосед.
   Тогда CAP переводится в station-UV соседа и их общий edge поглощается;
   fold, несовпадение shared geometry и неоднозначный CAP сохраняются.
   **Результат RF23:** multi-edge контакт с одной SEGMENT-face больше не
...
3. **Полевой набор §0d.3b + ярусы §0d.3a** — собрать headless
   ...
   **Результат:** DONE в R1.2 (`artifacts/decal_r12_field_*.png`,
   шесть объектов; atlas_frozen отделён от яруса-2 в
   `pytest.ini`).
4. **GPU-merge — ТЕКУЩАЯ ЗАДАЧА** ветки
   `claude/decal-gpu-preview-f3` (F3 готов,
   353 passed): rebase, интеграция с rail-путём, текстурный режим
   приоритетен; ФИКС world/local бага — оверлей рисуется через
   matrix_world объекта (при несброшенных трансформах превью не
   на месте меша). FPS drag — отложен (после R4).
4b. **R1.3 — RR9a: перпендикулярный снэп терминального
   направляющего** (полевые скрины пользователя после R1.2;
   СКОУП ФИНАЛИЗИРОВАН ответом пользователя):
   при >= 2 ВНЕШНИХ seam-кандидатах в конечной вершине spine
   направляющим торца выбирается ближайший к каноническому
   перпендикуляру (мера: argmin |dot(unit(кандидат),
   unit(spine-ребра))| в 3D; ничья -> плотина; mark сильнее
   снэпа). На fold-рёбра снэп НЕ распространяется; селектор
   ЕДИНЫЙ для планарного и криволинейного режимов (на кривизне
   вступает в силу с R3-материализацией; отдельного механизма не
   заводить) — см. «Финальная структура» в RR9a.
   Обязательная часть среза: торцы ФАКТИЧЕСКИ СКОЛЬЗЯТ вдоль
   снэпнутого внешнего seam-rail при preview-drag ширины
   (S1: план статичен, drag меняет extent) — «подтянуть движение
   торцов по крайним rails» (формулировка пользователя).
   Фикстура RF18. Малый срез сразу после GPU-merge.
   **Результат R1.3 + GPU-merge:** IMPLEMENTED (`2af6824`),
   синтетика/ярусы зелёные, GPU_TEXTURED default, matrix_world
   фикс (4.86 -> 2.98e-8). ПОЛЕВАЯ ПРИЁМКА НЕ ПРОЙДЕНА: торцы
   на объектах пользователя не скользят — их компоненты
   маршрутизируются на production developable-путь, который
   rail-план не читает (диагноз ревью, RM9). Не дефект
   исполнителя — пробел скоупа задачи.
4c. **R1.4 — Terminal-bridge (RM9) — ТЕКУЩАЯ ЗАДАЧА:** торцы
   production developable-пути читают терминальные выборы из
   rail-плана (read-only, один источник, без второго селектора);
   скоуп строго торцы, интерьер бит-идентичен; этап 1 —
   диагностическая строка routing-отчёта (per-торец: PCHAIN id /
   FOLD id / PERP + backend) и таблица по шести объектам; этап 2 —
   потребление плана торцами. Фикстура RF19; приёмка — полевые
   скрины пользователя (синие стрелки, арка + склон, две ширины).
   Невыразимость скольжения в торцевой механике production-пути
   без крупного рефакторинга — именованный дизайн-стоп.
   **Результат R1.4:** мост работает (арка/sagging по 4 PCHAIN,
   building RAIL_PLANAR 16/16, ярус-2 500 passed), НО полевой стоп
   пользователя: скольжение реализовано НАКЛАДКАМИ — отдельные
   faces поверх ленты, двойное покрытие терминальной зоны.
   Первопричина — противоречивая директива ревьювера (признано,
   RM9-fix в оракуле).
4d. **R1.4-fix — Торец есть срез, не накладка (RM9-fix) —
   ТЕКУЩАЯ ЗАДАЧА:** переразрез концевых faces ленты по
   терминальному rail (срез ЗАМЕНЯЕТ перпендикуляр, накладка
   запрещена); ноль двойного покрытия в терминальной зоне;
   бит-идентичность — только для faces вне терминального среза;
   RF19 с негативом «нашлёпки»; обязательные wireframe/edit-mode
   скрины терминальных зон (shaded маскирует дубли). Приёмка —
   пользователь в edit mode на своей сцене.
   **Результат R1.4-fix:** IMPLEMENTED (`9b72be0`), принят ревью:
   переразрез в production-пути (`decal_voronoi.py`), накладочный
   слой удалён, шесть объектов x две ширины — перекрытий 0,
   ярус-2 504 passed. Особое мнение исполнителя (первое по §0f)
   ПРИНЯТО: headless-инвариант нулевой площади перекрытий — в
   обязательный протокол полевой приёмки;
   `TERMINAL_BRIDGE_CUTS_OVERLAP` канонизирован (RM9-fix п.5).
   Остаётся ПОЛЕВАЯ ПРИЁМКА ПОЛЬЗОВАТЕЛЯ в edit mode — до неё
   торцевая тема не закрыта; исполнитель тем временем переходит
   к R2 (п.5), при провале приёмки фикс возвращается hotfix'ом
   вне очереди.
   **Полевая приёмка ПРОВАЛЕНА («клювы»):** после `a31972a` декаль
   растёт с торцов вдоль направления цепочки. R2 приостановлен.
4e. **R1.4-fix2 — «Клювы» (RR9b) — ТЕКУЩАЯ ЗАДАЧА:**
   1) диагностикой RM9 подтвердить на кейсе пользователя (частично
   выделенная цепочка хребта), что гидом торца выбрано продолжение
   собственной pChain; бисекция `9b72be0` vs `a31972a` — на каком
   коммите появились клювы; 2) реализовать RR9b: кандидат,
   инцидентный граням обеих сторон (продолжение-класс), — не гид,
   а делимитер; гид стороны обязан лежать в веере граней этой
   стороны (чистая топология, без углов); 3) `a31972a`
   (поглощение короткого site) — принят условно, закрепить
   RF19-кейсом; 4) фикстуры RF20 (клюв-негатив + L-поворот
   позитив) и RF21 (цепочка до границы меша); 5) ярус-2 ОБЯЗАН
   завершиться: таймаут ≠ зелёный (§0d) — тяжёлые atlas-тесты,
   просочившиеся мимо `atlas_frozen`, пометить маркером
   (конфиг-изменение = кодовое, один пере-прогон легален) и
   выяснить в отчёте, ПОЧЕМУ просочились. Wireframe-скрины торцов
   до/после, две ширины.
4f. **R1.4-fix3 — односторонний boundary-spine (RR9b/RF22) —
   ТЕКУЩИЙ ПОЛЕВОЙ HOTFIX:** выбранная seam сама лежит на mesh
   border (одна owner-грань), а не лишь упирается в него, как RF21.
   Соседнее одностороннее ребро той же source-pChain — продолжение
   контура и делимитер, не терминальный гид; без иных кандидатов
   торец = PERP. Чужие border/pChain-кандидаты не подавлять.
   Приёмка: RF22 без роста вдоль chain и без клювов; RF21 остаётся
   зелёным; поле пользователя на двух ширинах.
4g. **R1.4-fix4 — общий reach торца (RR9c/RF25): ОТОЗВАНО.**
   Самокритика исполнителя (§0f) принята ревью: `7170410` —
   недоказанная компенсация (связывает стороны без инварианта,
   режет здоровую длинную сторону, нет «до/после» на отмеченных
   вершинах). Коммит откатить; RR9c приостановлен в оракуле.
   `401490e` (CAP absorption) остаётся: ревью кода подтвердило —
   геометрия не меняется (только UV-переатрибуция при структурном
   условии единственного соседа с идентичным affine-map).
   **`7b7c492` (BEVEL) — ОТКАТИТЬ, полевой стоп пользователя
   подтверждён ревью кода:** classify_corner_runtime и
   _classify_extrusion_angle при join_mode=BEVEL подменяют не
   только MITER, но и KITE (вогнутые/reflex-углы) — треугольный
   кроп на вогнутом угле структурно чужой («corner там, где не
   должно быть»), вопреки собственной записи «остальные corner
   policies не меняются». Пересдача BEVEL позже: BEVEL применим
   ТОЛЬКО там, где применился бы MITER (выпуклый класс); негатив
   RF24: reflex/KITE-угол НИКОГДА не получает BEVEL; wireframe
   обоих классов углов.
4h. **R1.5 — Скольжение контакта по boundary (RR8b) —
   IMPLEMENTED / FIELD CANDIDATE (`e98c5ba`):** полевой стоп: при
   сильном росте контакт на boundary
   seam edges ведёт себя неверно; нижняя юбка упёрлась, верхняя
   скользит. Этапы: 1) откат `7170410` И `7b7c492` (bevel, см.
   4g); 2) провенанс-диагноз по
   протоколу исполнителя: отмеченные пользователем вершины на двух
   ширинах -> provenance каждого движущегося/застрявшего key:
   `TERMINAL_*`+station (торцевой rail) / `BODY`/`SOURCE_EDGE`
   (боковой alpha-фронт по границе domain) / CAP/KITE (corner
   ownership); 3) структурный фикс по исходу, критерий — инвариант
   RR8b (contact(alpha) = станционная точка boundary-route,
   монотонна; ровно две легальные остановки; единый механизм обеих
   юбок; проверить одностороннюю ветку атрибуции для border-рёбер);
   4) фикстура RF26 (три ширины, обе юбки). Отчёт: таблица
   вершина->provenance->позиция(w1,w2), wireframe до/после.
   **Результат:** `7170410` откатан `02fa376`, `7b7c492` откатан
   `c7194c1`, `401490e` сохранён. Диагноз на `sagging_wall/e3`:
   дефект = `BODY/SOURCE_EDGE`; reach двух in-plane paths
   1.447880990 / 2.729303861, но старый evaluator обрезал весь
   channel первым reach. Исправление размыкает пределы берегов:
   короткий остаётся на route-end, длинный продолжает до своего
   доказанного reach; контакт с односторонним border получает
   канонический `rail-source-edge-station`, без второго
   `rail-frontier-cell`. RF26 и полевой provenance:
   `artifacts/decal_r15_boundary_contact_report.md`; wireframe:
   `artifacts/decal_r15_boundary_contact_wireframe.png`. Финальная
   полевая приёмка остаётся за пользователем. Ярус-2 после кода:
   516 passed / 3 skipped / 4 deselected (`not atlas_frozen`),
   завершён за 81.29 s; тяжёлые atlas-тесты из маркера не
   просочились.
   **Ревью: ПРИНЯТО** (код проверен: откаты зеркальны, размыкание
   пределов корректно, ключ контакта единожды вычисляем — один
   источник). Задекларированный остаток: каналы, структурно
   связанные с corner-partition, ПОКА сохраняют общий
   `alpha_limit` — легальный переходный кламп до R4-параметризации
   углов; снять при R4, регресс — обе юбки RF26 у угла.
   Финальный арбитр — пользователь в Blender; при провале — hotfix
   вне очереди. ~~R2 ВОЗОБНОВЛЁН~~ **Полевая приёмка провалена по
   трём пунктам (скрин со стрелками 1-3) — R2 снова на паузе.**
4i. **R1.6 — Полевой hotfix-срез по трём стрелкам — ТЕКУЩАЯ
   ЗАДАЧА:**
   **A. Стрелка 1 — сдвиг контакта внутрь у boundary.** Гипотеза
   ревью: это задекларированный corner-остаток (`alpha_limit`-кламп
   каналов corner-partition) — стрелка указывает на УГОЛ контура.
   Диагноз на сцене пользователя (per-corner provenance двух
   ширин): если кламп — остаток объявляется полевым-неприемлемым,
   независимые reach сторон внутри corner-partition реализуются
   СЕЙЧАС (вперёд из R4); если структурная параметризация угла
   этого не выражает — именованный дизайн-стоп с фактами, не
   компенсация. Если НЕ кламп — таблица provenance, стоп. RF26b:
   юбки у угла контура, три ширины.
   **B. Стрелка 3 — Corner Join BEVEL: пересдача.** Фича была
   откатана как дефектная (треугольник на чужом углу), НЕ забыта.
   Пересдать правильно: BEVEL только для выпуклого MITER-класса,
   по переключателю, без влияния на KITE/reflex (негатив RF24);
   wireframe обоих классов углов. Если пользователь хочет
   треугольник и на вогнутом угле — отдельное дизайн-обсуждение,
   не самодеятельность.
   **C. Стрелка 2 — CAP не поглотился в strip-UV.** Условие
   поглощения ИЗМЕНЕНО ревью (см. поправку в оракуле после RF12):
   копланарность заменена станционной UV-непрерывностью — fold
   поглощению не мешает; каждый непоглощённый CAP несёт counted
   `CAP_KEEP_<REASON>`. Диагноз конкретного бирюзового CAP
   пользователя: назвать причину, по которой он не поглотился
   старым условием, и показать поглощение новым. RF23 обновить.
   Отчёт: по каждой стрелке — до/после на сцене пользователя
   (та же сцена/выделение, sagging_wall), две ширины, wireframe +
   UV-острова для C.
   **Статус R1.6:** A — стоп исполнителя ПРИНЯТ (гипотеза
   corner-клампа опровергнута: production-путь, corner_partitions=0;
   реальный механизм — остановка стороны на конце ЦЕПОЧКИ e9 при
   продолжающемся контуре). Дизайн-решение ревью: **RR8c** —
   ...
   **Ревью R1.6: ПРИНЯТО по коду** (`3e0fdeb` проверен:
   continuation-события counted, станции насквозь в ОДНОМ route —
   второго представления нет, развилка -> именованный DAM).
   Финальный арбитр — live edit-mode проверка пользователя по трём
   стрелкам на той же сцене/выделении; R2 стартует только после её
   зелёного вердикта.
   **Полевая приёмка ПРОВАЛЕНА по двум пунктам (скрины с красными
   стрелками):** обе юбки теперь уплывают внутрь меша (хорда вместо
   контура); BEVEL перепутал углы (целевой не изменился, чужой
   получил вросший треугольник).
4j. **R1.7 — Хорда-торец и BEVEL-инверсия — ТЕКУЩАЯ ЗАДАЧА
   (гейт 0d.0 обязателен):**
   **A. Хорда (RM10):** граница ленты между станциями контакта
   обязана лежать НА рёбрах контура; каждая пройденная вершина
   контура эмитируется вершиной ленты со своим `rail-source-vertex`
   ключом. Диагноз: показать на v10-маршруте e9->e27->e26, какие
   вершины контура НЕ эмитированы сейчас; фикс в
   production-evaluator; RF27-негатив 3.
   **B. BEVEL-инверсия:** пер-угловая таблица на сцене пользователя
   (vertex id, is_convex, policy, скрин-кроп) — целевой выпуклый
   MITER остался квадом, чужой угол получил «вросший» треугольник.
   Гипотеза ревью: convexity/winding вычисляется в ориентации,
   зависящей от знака нормали/обхода домена -> инверсия на этой
   поверхности. Фикс обязан быть ориентационно-инвариантным
   (convexity относительно обхода САМОЙ ленты); негатив RF24:
   классификация углов бит-идентична при флипе нормалей source-меша
   (инвариант 2). «Вросший» (утопленный в геометрию) треугольник —
   отдельный именованный дефект winding/side.
   **Конструктивная спецификация BEVEL (директива по скрину
   пользователя с красной полоской; заменяет реализацию через
   crop-hull):** у угла ленты в вершине V соседние сегменты уже
   имеют внешние offset-точки P1 = V + alpha*n1, P2 = V + alpha*n2
   (те самые, из которых MITER строит апекс). BEVEL = апекс НЕ
   вычисляется вовсе; угловой кусок = ПРЯМОЙ треугольник
   (V, P1, P2); красная полоска пользователя = ребро (P1, P2).
   Никакого convex-hull/клиппинга для этого куска. Выбор стороны —
   БЕЗ знаковой математики: в углу одна сторона ленты имеет ЩЕЛЬ
   между соседними квадами (внешняя -> join: miter или bevel),
   другая — ПЕРЕКРЫТИЕ (внутренняя -> clip); щель/перекрытие
   определяется пересечением кромок соседних квадов — тест не
   зависит от знака нормалей и направления обхода, инвертироваться
   нечему. Терминология UI: join styles Miter (default) / Bevel
   (переключатель), семантика stroke-linejoin.
   **Соответствие исходному дизайну (проверено по прототипу
   пользователя):** в pyvoronoi-прототипе угол — ПЕРВОКЛАССНАЯ
   точечная ячейка диаграммы (`source_category` 1/2 ->
   `point_source`, ячейка вершины между двумя сегментными);
   join-стили — три способа нарисовать внешнюю границу ЭТОЙ ЖЕ
   ячейки: FAN=дуга, MITER=апекс, BEVEL=хорда (основание клина).
   BEVEL реализуется как заполнение СУЩЕСТВУЮЩЕЙ угловой ячейки,
   не как новый вид геометрии. Два обязательных уточнения:
   (i) P1/P2 ЧИТАЮТСЯ из уже эмитированных внешних углов соседних
   квадов (пост-клип, общие ключи) — не пересчитываются заново;
   поэтому «без клиппинга» корректно ПО ПОСТРОЕНИЮ и треугольник
   не может торчать за контур; (ii) provenance куска — стандартная
   угловая identity RM6a (угловая вершина, spine-ребро, сторона),
   тот же слот политики `_CornerPolicy.BEVEL` — никакого
   ...
   **Результат R1.7:** IMPLEMENTED (`dc68d6d` RM10 contour-route
   без хорды; `b4dca39` BEVEL native point-cell, поле: только v8,
   v9 нетронут; ярус-2 522 passed). Полевой тест пользователя:
   RM10 и BEVEL-на-своём-углу подтверждены; НАЙДЕН следующий
   дефект — «веер вершин»: предрасчёт столкновений держит
   FAN/дуговую форму угла, эмиссия — BEVEL-хорду (два
   представления, RC5).
4k. **R1.8 — Столкновения читают эмитированный угол (RC5/RF28) —
   ТЕКУЩАЯ ЗАДАЧА:** 1) диагноз на сцене пользователя (стена с
   проёмами): подтвердить, что выделенный «веер вершин» —
   дискретизация угловой дуги в arrangement/precalc, а не эмиссия;
   2) угловые-локальные структуры столкновений читают join style
   (единый источник формы угла — эмитируемая ячейка); переключение
   MITER<->BEVEL перевыводит ТОЛЬКО их (локально, без полного
   recompile; контракт RF24 скорректирован в RC5); 3) пустой клин
   за хордой — свободное пространство для чужого потока; 4) RF28
   (+негатив веера). Отчёт по гейту 0d.0 на сцене пользователя.
   R2 — на паузе.
   **Дополнение (полевой стоп «врезается на широкой ширине»,
   скрины MITER-ок/BEVEL-кламп; подтверждено ревью кода):** BEVEL
   зарегистрирован в `explicit_corner_indices` классом
   SMOOTH/ACUTE_SPLIT с `corner_crops=()` — arrangement выделяет
   углу растущую вороную территорию, режет по её биссектрисам
   соседние сегменты, а покрытия territory нет -> «невидимая
   стена». РЕШЕНИЕ-ИНВАРИАНТ: **BEVEL = MITER во всём, кроме
   заполнения углового куска.** В классификации arrangement,
   кропах, лимитах и столкновениях BEVEL идёт ПУТЁМ MITER
   (никакого explicit-membership, никакого подавления coverage);
   единственная легальная ветка `policy == BEVEL` — эмиссия
   углового куска: треугольник-хорда вместо апексного полигона.
   Любая другая BEVEL-ветка в пайплайне — именованное нарушение.
   ...
   **R1.8 ПЕРЕОТКРЫТ как архитектурная переделка (стоп
   пользователя + самодиагноз исполнителя: `join_mode="MITER"`
   захардкожен в arrangement, decal_voronoi.py:4540; тест
   «SEGMENT BEVEL==MITER целиком» — неверный контракт).**
   Противоречие директив ревью признано и исправлено — читать
   **RC5a** в оракуле: (a) join не меняет СВОЮ ленту (собственные
   сегменты идентичны — анти-кламп регресс сохраняется);
   (b) материя угла для ВСЕХ остальных = эмитируемая форма
   (хорда/апекс) из одного источника point-cell; чужая граница
   доходит до хорды; клин — ничья территория. Единая угловая
   модель, параметризованная join'ом; две параллельные реализации
...
4l. **R1.9 — Насыщение на экстремальной ширине (RR8d):** полевой
   ...
   этой ширине — исчерпание/намотка на замкнутом контуре/
   пересечение со срезом другого торца; 2) реализация RR8d:
   исчерпание = насыщение (клامп на последней станции, counted),
   revisit-guard на замкнутом контуре, встреча контактов —
   станционное равенство (одномерный RC1); ошибка остаётся только
   для истинно неконструируемого; 3) preview==confirm: единая
   валидация — если preview проходил ширину, падавшую на done,
...
6. **R3 — кривизна**: сфера облегает (RF2, полюс-CAP RM5);
   ...
   здесь.
7. **R4 — UV из rail-домена**: (s,r)->(U,V) по существующему
   контракту; G3-admission для качества UV.
8. UX: кресты-хелперы при drag НЕ убирать (информативны);
   приложить ЛЕГЕНДУ оверлея к ближайшему отчёту; R0-подсветку
   позже за явный тумблер, не удалять.
...
# 0f. Голос исполнителя (решение пользователя после RM9-fix)

Пользовательское решение: исполнитель — сильная фронтирная модель
и НЕ обязан супер-жёстко следовать директивам; у него есть зона
собственных решений и право на своё видение. Урок RM9: буквальное
исполнение противоречивой директивы (наложение вместо вопроса)
стоило итерации; молчаливая исполнительность заведомо кривого
указания — теперь САМА ПО СЕБЕ процессный дефект.

1. **Обязанность возражать.** Директива выглядит противоречивой,
   вредной или хуже очевидной альтернативы -> исполнитель ОБЯЗАН
   сказать это: либо дизайн-стоп с вариантами ДО реализации, либо
   (для малого) реализовать лучший вариант и ЗАДЕКЛАРИРОВАТЬ
   отклонение в отчёте с обоснованием. Оба пути легальны;
   нелегально только молчаливое буквальное исполнение брака.
2. **Свобода реализации.** Всё, что не меняет семантику канона —
   структуры данных, алгоритмы, порядок работ внутри среза,
   инструментарий, тесты сверх минимума, диагностика — на
   усмотрение исполнителя без согласований.
3. **Предложения по канону.** Правки правил RR/RM/RC/RP исполнитель
   ПРЕДЛАГАЕТ отдельным блоком отчёта «Особое мнение /
   предложения»: проблема -> вариант -> что меняет. Ревьювер
   обязан ответить решением по существу. Видение архитектуры
   целиком — тоже приветствуется этим каналом.
4. **Что остаётся жёстким (шрамы проекта, не кляп):** запрет
   числового тюнинга поведения; запрет второго представления
   одного объекта; запрет ТИХИХ отклонений от канона (отклонился —
   задекларируй); полевая приёмка пользователя — финальный арбитр.
   Эти четыре пункта — единственные, где буква обязательна.
5. **Сигнализация РИСКОВ (дополнение пользователя при передаче
   рефакторинга).** Обязанность замечать и НАЗЫВАТЬ риски до
   того, как они станут дефектами: ошибки/неточности/неполноту в
   ТЗ ревьювера, риски собственной реализации, риски соседнего
   кода, на который опирается задача. В каждом отчёте —
   стандартный блок «Риски»: что может сломаться, что
   предположено, что не проверено. Пустой блок — явное «рисков не
   вижу», а не пропуск блока.
6. **Люфт и малые пивоты.** В рамках задачи исполнитель ВПРАВЕ
   решать о небольших пивотах (подход, порядок, объём внутри цели
   среза), если это идёт на пользу цели, — с декларацией в
   отчёте: что изменено и почему. Граница люфта: контракты брифа,
   четыре шрама п.4 и геометрический канон; пивот, выходящий за
   них, — дизайн-стоп с вариантами. «Бездумное» буквальное
   исполнение директивы — само по себе процессный дефект
   (доказано дважды: нашлёпки RM9, BEVEL=MITER в столкновениях —
   обе ловушки создало буквальное исполнение кривых директив
   ревьювера).

---

# 0g. TRANCHE S — структурное переоформление движка декалей
# (УТВЕРЖДЁН пользователем: «перейти сразу к рефакторингу» после
# пятой неудачной итерации BEVEL; активен)

**ОНБОРДИНГ НОВОГО ИСПОЛНИТЕЛЯ: входная точка —
`docs/decal_engine_refactor_brief.md`** (карта потоков данных,
ревизия инвариантов, каталог болезней, контракты, программа
срезов, рабочие соглашения). Разделы §0a-0c этого файла — ИСТОРИЯ:
задачи оттуда не брать. Действующие: §0g (очередь), §0d
(тестирование + гейт 0d.0), §0f (голос исполнителя), §0e
(LEGACY-CUT, п.1 заменён S0).
...
- **S-CM — CornerModel (вставлен по решению о пивоте; формулировка
  исполнителя возведена в контракт):** ЕДИНАЯ пост-клип модель угла
  — V + P1/P2, прочитанные из уже клипнутых внешних углов
  собственных strip-сегментов, + join style — единственный источник
  для collision domain, ownership, UV, projection и 3D-эмиссии.
  Не сырой MITER point-cell, не визуальная нашлёпка, не два
  представления. BEVEL возвращается в UI ТОЛЬКО через CornerModel;
  приёмочный негатив: финальная спроецированная 3D-грань BEVEL
  имеет РОВНО 3 вершины (тест на итоговой геометрии, не на
  промежуточном crop — урок фальшивого отчёта `3a37745`);
  перекрытие соседних углов на большой ширине — через общую
  конкуренцию, не прямым расширением треугольника (попытка
  отвергнута исполнителем корректно). До S-CM: BEVEL-toggle
  ВЫКЛЮЧЕН из UI (честная недоступность вместо шестого костыля),
  MITER — дефолт; новые точечные фиксы угла на старой структуре
  ЗАПРЕЩЕНЫ.
...
**AMENDMENT по внешнему аудиту пивота (принят ревью, внесён в
бриф):** порядок уточнён — S-guard -> S0a -> S0b -> S1+S2a ->
S-CM.a -> ревизия заготовок -> R2+S-CM.b -> S2b -> R3 -> S3;
шесть редакций брифа (PatchGraph сохраняет solve-facing facts;
AnalysisBundle/SourceRevision + cross-IR инварианты; S0 =
semantic no-op с разрешённым выносом общих хелперов и
differential-критерием; фазировка CornerModel; «3 вершины» —
семантический контур + планарный оракул; S2 разрезан на
...
**ТЕКУЩАЯ ЗАДАЧА — ГЕЙТ R3-БЭКЕНДА** (короткий дизайн-документ,
БЕЗ production-кода): предложение авторитета ШИРИНЫ на гладкой
кривизне по данным S-WF0 (FMM — кандидат, Heat дисквалифицирован,
точные окна — референс): интеграция с rail-каноном (RM1 эмиссия
по source-граням, станции/(s,r), RP-скоуп planar/curved,
детерминизм и ретриангуляционная инвариантность), карта изменений
и рисков. Решение — пользователь + ревью; после — R3 (сфера).
...
`DECAL_CORNER_JOIN_ARCHIVED_UNTIL_CORNER_MODEL` (UI-disable
недостаточен: .blend/скрипты); (2) архивную ветку ЗАПУШИТЬ в
origin + immutable annotated tag (локальная ветка в контейнере
умрёт вместе с ним); (3) написать
`docs/decal_archived_modes_product_contract.md` ДО физического
удаления builders (S0b); (4) S0b по differential-критерию брифа;
(5) выяснить маршрут ветки: коммит сделан на `work` контейнера —
довезти в канон через PR.

УТВЕРЖДЁННЫЙ порядок (скорректирован пивотом): **R1.8
ПАРКУЕТСЯ** (не дозакрывается точечно: `3a37745` полевую приёмку
не прошёл — финальные BEVEL-грани 5-6 вершин, отчёт признан
исполнителем ошибочным; BEVEL выключается из UI до S-CM) ->
**S0** (архив-cut) -> **S1** (контракт поверхности) -> **S-CM**
(CornerModel; закрывает R1.8 структурно) -> проверка статуса
R1.9/R2-заготовок (`3d1abd8`, `94bb679` — перепроверить и принять/
переделать на новом фундаменте) -> **R2** -> **S2** -> **R3** ->
**S3**. Обоснование: урок GL «не строить выше на неверном
фундаменте»; пятая итерация угла на старой структуре — эмпирика
её сопротивления.
...
2. **Отказ обязан быть информативным и безопасным.**
   ...
   Автоподсветка русла при fail ОТМЕНЕНА пользователем (тест 5) —
   ручной R0-подсветки достаточно; отдельный код для неё не писать.
...
**0d.0 — Гейт визуальной сверки (введён после трёх подряд полевых
провалов при зелёных счётчиках).** Полевой hotfix, привязанный к
аннотированному скрину пользователя, считается report-ready ТОЛЬКО
при наличии в отчёте: (i) воспроизведённого рендера ТОГО ЖЕ ракурса
и той же сцены/выделения; (ii) по-стрелочного сравнения — для КАЖДОЙ
стрелки пользователя явная строка «было -> стало» с указанием, что
именно видно на рендере. Счётчики, синтетика и station-дампы гейт НЕ
проходят: только глаза по картинке. Отчёт без этого блока — не
отчёт (возврат без ревью).
...
3a. **Ярусы прогонов (баланс скорости, утверждён пользователем,
   тест 5):**
   - Ярус 0 (после каждой правки): ТОЛЬКО тесты затронутого
     модуля/файла (`pytest <файлы>` / `-k`), секунды.
   - Ярус 1 (перед коммитом): тесты затронутых модулей + 1-2
     сцены полевого набора.
   - Ярус 2 (ОДИН раз на срез, перед push): полный suite + весь
     полевой набор со скринами.
   «Все тесты на чистую» после каждой правки — запрещено; полный
   suite не перегоняется без изменения кода. Осознанный риск:
   регрессия может доехать до границы среза — принят.
   ОРГАНИЗАЦИЯ ЯРУСА-2 (директива после timeout-стопа R1.1):
   замороженные атласные тяжёлые тесты (GL-pending; в т.ч.
   многоминутные saddle-материализация/детерминизм) выносятся за
   pytest-маркер `atlas_frozen` и в ярусе-2 НЕ выполняются —
   замороженный код не меняется, его тесты не могут регрессировать;
   их полный прогон возвращается один раз при возобновлении GL.
   Ярус-2 = полный suite БЕЗ `atlas_frozen` + полевой набор; обязан
   ЗАВЕРШАТЬСЯ (timeout без напечатанных падений НЕ считается
   зелёным — это отсутствие свидетельства, а не свидетельство).
   Однократный перезапуск после введения маркера легален (изменение
   конфигурации = изменение кода).
3b. **Состав полевого набора (пользователь, тест 5):**
   rounded_wall.001, rounded_wall_noise_top (шумный непланарный
   верх, сложная топология), sagging_wall, wall_noise_top,
   half_sphere, building. Брать их seam edges и прогонять; вывод —
   СКРИНШОТЫ (дешевле по токенам); выгрузка данных — только когда
   скрин не отвечает на вопрос.
