...
4g. **R1.4-fix4 — общий reach торца (RR9c/RF25):** при сильном
   росте две patch-стороны одного физического торца больше не читают
   разные пределы terminal route. Общий station cap = минимум route-reach
   сторон; достигшая boundary сторона и более длинная сторона останавливаются
   вместе. PERP/односторонняя ROUTE не меняются; порогов нет.
