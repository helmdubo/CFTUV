...
РЕВИЗИЯ ОЧЕРЕДИ (утверждена пользователем): полевая ценность
поднята ПЕРЕД довершением E-acceptance; атлас уходит на чекпойнт и
возвращается одним консолидационным срезом GL (Geometry Ledger)
вместо по-стопового режима. Обоснование: ~70% стопов W1 — одна
болезнь («один идеальный объект — несколько независимых
представлений»); полевые дефекты (расшитие C8.4, юбка G2/G3) атласа
не требуют; атлас default-off и пользователям не виден.

Выполнено: W-CP (чекпойнт атласа, `GL-pending` xfail), W3
(C8.1+C8.4), W4-замер (+валидированная диагностика `febfa2b`),
C8.3, C8.2, G1 (`723607d`), C8.5 (`bb6a9d7`), G1.1 (needle-root:
SMOOTH arrangement, не imprint), W4-fix (`2bfa4bd`, монотонный
sweep). G3-решение принято. Актуальная очередь:

ПИВОТ (утверждён пользователем, тесты 3-4): геометрия лент
переводится на rail-архитектуру — спецификация в
`docs/decal_rails.md` (rail-оракул, канон после утверждения).
Визуальный контракт V1-V3 сохраняется как определение готовности
и закрывается срезами R: V1+V2 -> R1, V3 -> R3.

1. **C8.6** — materialization fail на большом меше (hard fail
   ТЕКУЩЕГО пути; чинится в текущем коде до пивота, секция в
   C8-блоке).
2. **R0 — Трассировка русел + подсветка.** RR1-RR7 как
...
3. **R1 — Материализация PLANAR по руслам** (стена): RM1-RM6 на
   ...
   G1.2 и C8.7. Намеренная пересдача differential. Предписания
   по стопу R0->R1 (внесены в оракул): станция-0 плотина —
   виртуальный rail RM5a (taper запрещён; PLANAR-only, на
   кривизне до R3 — стоп); координаты — нейтральные (s, r) по
   RM2-поправке, публичный UV-контракт не меняется до R4; углы —
...
5. **R3 — Криволинейные поверхности** (сфера): RF2 (облегание),
   RM5 полюс-CAP; закрывает V3; поглощает G2.
6. **R4 — UV-слой из rail-домена**: chart строится ИЗ русел
   (один источник); G3-admission применяется к КАЧЕСТВУ UV
   (measured-first, решение принято); knob-рекомендация для
   широких лент.
7. **Merge GPU-ветки** `claude/decal-gpu-preview-f3` — после R1
   (превью рисует любые faces, чек-лист обновить).
8. **Пересмотр атлас/GL/W2/E-acceptance** — ПОСЛЕ R: объём
   атласной сложности пересматривается, т.к. chart теперь
   строится из rail-домена; решение ревьювера по итогам R3/R4.
9. **F1 (инкрементальность)** — по результатам производительности
   R-пути.

Каждый R-срез: отдельные коммиты, полный suite, отчёт со
скринами фикстур (полевые сцены пользователя — обязательная
часть acceptance), stop-условия из §7 оракула.

Параллельно (пользователь): ручная проверка rounded_wall drag в
...
Смешивать W1/W3-коммиты запрещено. После W3-пересдачи EP1 для
атласных коммитов считается против НОВОГО эталона.

СТАТУС (ревизия очереди, см. §0a): W1 исполнен до T7-P3.15 и
уходит на чекпойнт W-CP; W2 переносится ПОСЛЕ среза GL (§0c).
Порядок теперь: W-CP -> W3 -> W4 -> (C8.3/C8.2/G1/G3/G2/GPU по
§0a) -> GL -> W2.

## W-CP. Атласный чекпойнт

1. Закоммитить/запушить текущее атласное состояние (все правила по
   T7-P3.15 включительно, включая read-only provenance-трейс).
2. Saddle-acceptance тесты width 0.25/0.5 — xfail с маркером
   `GL-pending` и ссылкой на §0c (только acceptance-фикстуры
   атласа; юниты и негативы остаются строгими и зелёными).
3. Полный suite зелёный; EP1 байт-в-байт (worktree-diff); отчёт:
   baseline SHA, счётчики, список xfail-маркеров.
4. После W-CP атласный код не трогается до GL (исключение:
   конфликт при rebase других срезов — только механическое
   разрешение).
...
4. Регрессия на репро design-stop: saddle, width 0.25/0.5/1.0/2.0 —
   ...
   на O2 — проверка порядка обнаружения. Девятый кейс — честные
   декларации + авторитет кривых (T7-P3.10; width=0.5): (a) fuzzy
   `midpoint <= 1.5q -> SEGMENT` удалён, ассерт «никакая
   дистанционная эвристика не переписывает класс деклараций»;
   ПЕРЕСМОТР кейса 2 — фантомный хвостовой интервал 12032 больше
   не возникает, проверяется прямая декларация класса угла над
   истинным спаном, механика T7-P3.7 покрывается синтетической
   фикстурой с честным 1q-интервалом; (b) same-token class-change
   (chart8, token 12016 SEGMENT|KITE) требует сепаратор наравне со
   всеми — face разрезан, O2 зелёный; (c) по итогам диагноз-шага
   T7-P3.10c: либо третья ветвь гейта 3.8 (сепаратор в границе
   face -> континуация, guard зелёный без ослабления), либо
   requirement-стадия для transition-endpoint якоря по T7-P2.2(a);
   негативы прежних кейсов не деградируют. Десятый кейс —
   канонический ключ coverage-границы (T2-C.7; chart0, transition
   (22,44) 0<->2, raw frontier 713 против канонической границы
   712, 1q полоса «материализовано/owner=None»): после фикса
   декларация и T3-конформация frontier-кривой читают ОДИН
   канонический station-ключ — полоса структурно исчезает, face
   разрешается в 12002, T2-C зелёный; ассерт точного совпадения
   coverage-границ с T-якорями frontier-кривых на всех фикстурах;
   негатив: рассинхронизировать ключи искусственно ->
   `ATLAS_TRANSITION_DESYNC` ДО arrangement; ассерт «None-интервалы
   не поглощаются» (T7-P3.7 применим только к owner != None).
   Одиннадцатый кейс — целостность crop-циклов (A-INT; chart6,
   transition (26,48) 4<->6, face сквозь собственную crop-границу
   12010 поверх None [709,2828]): pre-trace guard замкнутости
   конформированных crop-циклов зелёный на всех фикстурах; после
   фикса face разрезан frontier'ом, T-touch внутренней части в
   границах [0,709], T2-C зелёный; негатив: искусственно удалить
   subedge crop-цикла -> `ATLAS_ARRANGEMENT_INTEGRITY` ДО tracing
   (не T2-C постфактум) — проверка порядка обнаружения;
   homogeneity-оракул (sampled вершины циклов против предикатов) —
   ноль нарушений, drop не используется. Двенадцатый кейс —
   замыкание transported-региона (A-INT.4-6; chart6, KITE12012:
   4 crop-edge imports с ПУСТЫМИ anchor_stations, внешняя химерная
   cycle между продолжениями imported-кривых поверх None
   [709,2828] transition (26,48)): после фикса —
   `_m1_build_semantic_imports` публикует anchor_stations через
   единый T2-ключ (включая выход региона на вторую transition),
   endpoint'ы imports перенесены в станции pre-arrangement;
   pre-trace guard замыкания «цепь + T-интервалы» зелёный на всех
   фикстурах; химера распалась: материальная face
   классифицирована владельцем, внешняя — no-owner drop без
   материализации, T2-C зелёный; негатив: опустошить
   anchor_stations искусственно -> `ATLAS_ARRANGEMENT_INTEGRITY`
   ДО tracing; ассерт «ни одной imported-кривой без якоря или
   стыка цепи» (T7-P.3) на всех фикстурах. Тринадцатый кейс —
   nesting-стадия (A-INT.7; chart6: внешняя cycle с material
   island 12010 внутри, representative попадал в остров /
   declared-touch с входной T): containment forest точен на
   B0-решётке, регион = цикл минус immediate children;
   регион-верный representative (вне children); после фикса
   no-matter кольцо касается T только None-интервалов -> counted
   no-owner без материализации, остров материализуется из своих
   циклов, T2-C зелёный; ассерты: в эмитированном IR только
   простые faces (hole-records запрещены); double-cover 0 на
   вложенных material-в-material фикстурах (синтетическая: малый
   crop строго внутри большого без пересечения границ);
   homogeneity-оракул сэмплирует регионы — ноль нарушений;
   негатив: отключить forest -> старая химера обязана
   воспроизвестись и упасть T2-C (санити регрессии).
   Четырнадцатый кейс — endpoint-requirement (T7-P3.11; chart6,
   tiny face у source vertex 5: KITE corner5 по (5,26) против
   SEGMENT 12010 по (5,27), boundary_corners=()): requirement-
   стадия per (chart, source vertex) обнаруживает смену класса
   вокруг узла, owner corner5 экспортирует kite crop-edge через
   единый endpoint-ключ (общий для обеих transitions), кривая
   продолжена по T7-P2.2 -> face разрезан, O2 зелёный; ассерт
   единственности якоря узла (endpoint-ключи всех инцидентных
   transitions резолвятся в ОДНУ source-vertex станцию); негатив:
   удалить endpoint-экспорт -> `ATLAS_CLASS_DESYNC` ДО
   arrangement, не O2 (порядок обнаружения); негатив «нет кривой
   владельца в пределах кванта у endpoint'а» (fault-injection) ->
   тот же fail-fast.
   Пятнадцатый кейс — метрика инцидентности точка-к-кривой
   (T7-P3.11e; chart13, узел source_vertex=35, почти-параллельная
   kite14|12028 граница: прежняя метрика через точку пересечения
   давала miss=500q = alpha - длина_T при суб-квантовом боковом
   зазоре): repro проходит по метрике точка-к-кривой, endpoint
   кривой перенесён в узел (суб-квантовый T3-move), face разрезан,
   O2 зелёный; ассерт «метрика инцидентности узлу не использует
   пересечений с T-line»; юнит-регрессия на sqrt(_dist2)-фикс
   (метрика на известных расстояниях); негатив: кривая с боковым
   зазором > кванта (fault-injection) -> `ATLAS_CLASS_DESYNC` до
   arrangement; первый endpoint-кейс (source_vertex=26) остаётся
   зелёным.
   Шестнадцатый кейс — region-level authority (T7-P3.12; chart14,
   три SEGMENT-subface вдоль T(35,57): corner14 в границе крайних,
   средний 1..5 с overrun 1.12q отрезан refinement'ом): регион
   (owner token, класс, связность через одинаково
   классифицированные рёбра) вычислен до guard'ов; средний face
   легален по границе региона (provenance corner id == endpoint
   vertex), V-континуация, guard зелёный; ассерт «регион не
   пересекает сепаратор» (утечка через реальный угол невозможна —
   негатив: face по ту сторону сепаратора НЕ получает continuation
   authority); ассерт chart-локальности региона (T-subedges
   связность не переносят); негатив: удалить сепаратор из
   границы региона (fault-injection) -> гейт падает как прежде.
   Семнадцатый кейс — ВЕРИФИЦИРОВАН, разрешение по T7-P3.14 (три
   ингредиента): (i) флан neighbor224 — двойная segment|kite
   граница (0.001q) -> канонизация 13c для пары, ассерт «ноль
   no-owner faces шириной <= 1q между копиями одной идеальной
   границы»; (ii) идентификация выполнена: НЕ сепаратор —
   разрешено через T7-P3.15; (iii) РАЗРЕШЕНО ЧЕРЕЗ T7-P3.15:
   полигон corner/KITE-образа транспортируется и регистрируется
   как election/classification-кандидат наравне с site image
   crops (T2-C.1 явно распространён на corner-образы); span 1..5
   chart14-side -> KITE corner14 (поправка предсказания 14c: не
   None), face236 классифицируется KITE с imported frame (R5),
   сегментный overrun/clamp для него исчезает; per-side принцип
   14c остаётся для настоящих касательных участков поверх ПОЛНОГО
   набора предикатов; ассерты: ни одного arrangement edge без
   provenance (closure-стабы помечены явно), corner-образы в
   pending-наборе на всех фикстурах, kite-язык материализуется по
   обе стороны T без двойного покрытия (T8/O1 на транзитных
   subedges kite-региона); негатив: убрать kite-кандидат из
   election -> ложная SEGMENT-декларация воспроизводится (санити);
   гейт 3.8, негативы T7-P3.12 и канонизация (i) не деградируют;
   carrier-probe/drop/merge не вводить (14d).
   Ассерт на всех фикстурах: в опубликованных T2-контрактах нет
   интервалов длиной <= 1 кванта (среди семантических; None вне
   правила поглощения).
...
# 0c. Срез GL — Geometry Ledger (консолидация атласного представления)

Мотивация. ~70% design-стопов W1 — инстансы одной болезни: один
идеальный геометрический объект существует в нескольких независимо
деривированных/квантованных представлениях (coverage-граница T2-C.7,
UV-frames R5, segment|kite ребро 13a, кросс-чартовые копии 14b,
corner-образ без полигона 15, безпровенансные стабы). GL устраняет
болезнь классом, а не инстансами. GL меняет ПРЕДСТАВЛЕНИЕ, не
семантику: все правила T7-P3.x / T2-C / A-INT / R5 остаются в силе,
их проверки переезжают на ledger-факты.

## GL-предрешения (не пере-обсуждать)

**GL1 — Один объект, один экземпляр.** Каждая идеальная кривая
компайла — crop-ребро любого cell-типа, class-сепаратор,
frontier-кривая, transition, closure-стаб — существует РОВНО ОДИН
РАЗ как ledger-объект: каноническая B0-геометрия, канонический
ключ, ПОЛНАЯ provenance (все идентичности сразу: пара смежных
ячеек, corner id, site/edge tokens, owner chart, роль), якорные
станции (A-INT.5), absorbed-флаг (T7-P3.8).

**GL2 — Потребители только читают.** Election (T2), transport/
imports, входы arrangement, классификация, guards ссылаются на
ledger-объекты по ключам. Локальная ре-деривация геометрии,
которая уже есть в ledger, запрещена — это обобщение «один
источник, много читателей» (T2-C.7/R2/R5) в один слой.

**GL3 — Общие границы едины.** Граница пары смежных ячеек — один
ledger-объект с обеими cell-идентичностями (13c). Кросс-чарт:
owner-версия авторитетна, consumer-crop ссылается на неё после
транспорта (14b). Corner/KITE-образы — первоклассные
election/classification-кандидаты (15): полигон образа
транспортируется той же изометрией, что его кривые.

**GL4 — Provenance тотальна.** Ребро arrangement без
ledger-provenance = `ATLAS_ARRANGEMENT_INTEGRITY` (расширение
A-INT); closure-стабы — ledger-объекты с явной ролью без классовой
нагрузки.

**GL5 — Семантика заморожена.** Никаких новых механик
классификации, допусков, коэффициентов, ослаблений оракулов.
Если кейс GL.3-чеклиста требует НОВОЙ семантики (не представления)
— стоп с полями, решает ревьювер.

## GL-слайсы

- **GL.0 Инвентаризация (отчёт до кода).** Аудит всех мест, где
  геометрия деривируется повторно (crop builders, import builders,
  election sampling, separator export, anchoring, стабы): таблица
  «идеальный объект -> сколько представлений сейчас -> где».
  Отчёт ревьюверу; короткое подтверждение до GL.1.
- **GL.1 Ledger.** Структура + единый компайл-проход построения;
  ключи/provenance/счётчики (`ledger_object_count`,
  `ledger_dual_representation_merged_count`).
- **GL.2 Переключение потребителей** по одному: election ->
  imports/transport -> arrangement input -> классификация ->
  guards. Каждый шаг — отдельный коммит, полный suite зелёный,
  EP1 байт-в-байт против актуального эталона.
- **GL.3 Чеклист стоп-хвоста.** Все 17 кейсов W1.4 переезжают в
  единый регрессионный набор поверх ledger; saddle width
  0.25/0.5/1.0/2.0 зелёный по полному оракульному набору (O1/O2/
  T8, double-cover == 0, components == 1, clamp == 0, все
  W1.4-ассерты); снятие `GL-pending` xfail-маркеров.

## Acceptance GL и переход к W2

Suite полный зелёный без атласных xfail; EP1 байт-в-байт; отчёт
(SHA, счётчики ledger, таблица GL.0 «было/стало»). Затем W2 —
полный E-acceptance по прежнему определению (§0b/W2): отчёт ->
короткое подтверждение ревьювером -> E объявляется закрытым в §0a.

## Stop conditions GL

- Объект, не вписывающийся в ledger-модель (дефицит
  выразительности GL1) — стоп с описанием объекта.
- Кейс чеклиста, требующий новой семантики — стоп с полями (GL5).
- Рост EP1-diff или деградация не-атласного suite на любом шаге
  GL.2 — стоп, откат шага.
...
Seams ВЫРЕЗАЕТСЯ из маршрутизации полностью — он маскирует поломки
rail/chart-пути и блокирует визуальное тестирование R-транша.
«Пусть ломается видимо» = fail-fast, канон проекта.

Правила исполнения:
1. **Вырезается МАРШРУТИЗАЦИЯ, не код.** Ни один выбранный
   seam-компонент не может уйти в LEGACY; сам legacy-строитель
   остаётся в дереве до отдельного cleanup-среза после R3
   (быстрый откат, если понадобится — только решением
   пользователя).
2. **Отказ обязан быть информативным и безопасным.**
   Неподдержанная геометрия -> явный fail с ИМЕНОВАННОЙ причиной
   (таксономия RAIL_*/CHART_*: напр. RAIL_CURVED_PENDING_R3,
   RAIL_STATION0_DAM_CURVED), причина в header/routing-строке,
   counted. Состояние сцены: PreviewStatus.ERROR по A6, никаких
   полусобранных/двойных объектов, выделение не разрушается.
   При fail АВТОМАТИЧЕСКИ включается R0-подсветка русла — артист
   ВИДИТ, где течение остановилось (плотины/полюса/причина).
3. **Top/Bottom-тримы не трогать вслепую**: «юбка» там — осознанная
   семантика (решение G2-эпохи). Если они делят строитель с
   legacy-Seams — разделить по режиму, а не выпилить оба. Спросить
   пользователя, если границы режимов неочевидны.
4. **Тесты**: существующие legacy-routing тесты переворачиваются в
   «ожидаемый явный fail с причиной», не удаляются молча; новые —
   только по §0d (репро). Намеренная пересдача differential для
   кейсов, которые раньше фолбэчили; здоровые PLANAR-маршруты
   остаются байт-в-байт.
5. **Полевой набор**: сцены, которые теперь честно падают (сфера
   до R3), фиксируются как expected-fail С ПРИЧИНОЙ — это видимая
   карта оставшейся работы, скриншоты подсветки прикладываются.

---

# 0d. Политика тестирования (арбитраж пользователя, обязательна)

Контекст: атласная сага W1 подтвердила риск «микрофиксы в цикле при
неверном фундаменте»; полевые тесты пользователя нашли все проблемы
уровня замысла. Пропорция усилий смещается к полю.

1. **Полевой набор мешей — первичная приёмка.** В репозитории
   хранится фиксированный набор реальных сцен (стена с аркой,
   rounded_wall, сфера, walls.010, большой меш C8.6; пополняется
   только полевыми кейсами пользователя) с seam-выделениями и
   ожидаемыми исходами. Прогон одной командой в headless Blender;
   АВТОСКРИНШОТЫ каждой сцены прикладываются к отчёту среза.
   Срез не принимается без зелёного полевого набора + скринов.
2. **Синтетика — только по репро.** Новый синтетический тест
   разрешён только с привязкой к конкретному дефекту (репро) или
   правилу оракула. Один баг -> один минимальный тест. Спекулятивные
   фикстуры — только с одобрения ревьювера. Существующие
   синтетические наборы не расширять.
3. **Без субагентных фан-аутов проверок.** На срез: один прогон
   полного suite + один прогон полевого набора. Многократные
   перепроверки/повторные прогоны без изменения кода запрещены.
4. Найденный в поле баг обязан оставить после фикса ровно один
   постоянный репро-тест — чтобы пользователь не находил его
   повторно.
...
Полевой репорт (арочная стена, декаль по гребню и кромкам): на
внешнем углу между верхней (арочной) лентой и левой — дырка.
Routing-строка получена (закрепляет триаж):
`PLANAR+INTRINSIC_DEVELOPABLE:1c/7e`, `chains:6 edges:7`, без
fallback'ов — строит CHART-путь (junction/fan/weld-слой Patch
Voronoi, НЕ legacy), бэкенд смешанный. Гипотеза №1 (уточнена
логом): КРОСС-БЭКЕНДНЫЙ junction — фан-сектор между лентой на
intrinsic-developable чарте (арка) и лентой на планарной
поверхности (стенка) не эмитится либо не сваривается; зона
`_synchronize_cross_surface_spine_stations` (односторонний sync,
известный по C8.2-контексту). Гипотезы №2-3: сектор фана на
выпуклом стыке двух chain'ов; CAP-зона после C8.1.
Триаж ДО фикса: headless-репро «арка ~90 град. на 8-10 сегментов +
вертикальные стенки», выделение как в тесте (сеть коротких
цепочек, junction на стыке PLANAR|INTRINSIC рёбер); подтвердить
строителя угла по provenance. Acceptance: на репро ноль дырок на
всех ширинах (в т.ч. полевые 0.74/1.27/1.61), junction покрыт
полным фаном через границу бэкендов, differential пересдаётся
осознанно.

Результат C8.5 (`bb6a9d7`): гипотеза №1 подтверждена. Runtime
junction-слой индексировал только PLANAR ports и поэтому не видел
открытый rail INTRINSIC-чарта. Для exact intrinsic port теперь
используется compile-static inward-направление чарта, отображённое
локальной изометрией в source surface; domain-transition может
обрезать луч раньше alpha-frontier. APPROXIMATE/atlas исключён.
Headless mixed-backend fixture (8 сегментов арки + PLANAR-стенка)
на ширинах 0.74/1.27/1.61: один connector-sector, обе rail-границы
имеют по два owner-face, один manifold component, preview/Apply
байт-идентичны.

### C8.6 Materialization fail на большом меше: repeated vertex keys

Полевой репорт (третья волна): большой меш, выделение ~450 рёбер,
`Decal materialization failed: backend=PATCH_VORONOI face=887
verts=5 component=SEGMENT reason=repeated vertex keys
repeated_keys=(('pv-se', 32, 0.9440057),)`. Fail-stop сработал
правильно (не тихая порча), но блокирует основной сценарий на
больших выделениях. Триаж ДО фикса:
1. Диагностический дамп при этом fail: все ключи face, геометрия
   вершин, СМЕЖНОСТЬ повторённой пары (соседние вершины цикла или
   несмежные/bowtie).
2. Происхождение дубликата `('pv-se', 32, 0.9440057)`: какие ДВА
   построителя выдали одинаковый парам на ребре 32 (квантование
   двух разных точек в один парам после C8.2 source-feature
   ключей? стык corner/crop-пересечения с сегментной станцией?).
3. Headless-репро: bisect выделения до минимального падающего
   подмножества.
Гипотезы-развилка: (a) дубликаты СМЕЖНЫ = вырожденное ребро после
квантования -> легальный collapse (T6-стиль, counted; face
становится 4-верт; видимая геометрия не меняется); (b) дубликаты
НЕСМЕЖНЫ = bowtie/самопересечение face — реальный дефект
crop/weld-построителя, чинить построитель, collapse ЗАПРЕЩЁН
(маскировка). Acceptance: репро зелёный на всех ширинах; ассерт
«нет повторённых ключей в эмитированных faces» остаётся жёстким
...
«небольших» углах; ожидание пользователя — цельная поверхность,
повторяющая ребро оригинала (бенд по source-ребру БЕЗ сплита).
Фактические фолды арки ~9-15 град. — ровно на границе SMOOTH
`tau <= 10`. Порядок (данные до порога): (1) замерить фактические
tau репро-фикстуры; (2) классифицировать источник сплитов —
corner-компоненты у границы SMOOTH ЛИБО needle-слайверы
G1-импринта (G1.1). Если corner-компоненты: SMOOTH-константа
поднимается ПО ТАБЛИЦЕ полевых углов решением ревьювера, не
самовольно; V-непрерывность и S1 обязаны сохраниться (angle-only
критерий остаётся).

Результат калибровки (`artifacts/decal_c8_3_calibration.json`):
первичная гипотеза «поднять порог» опровергнута. На headless-арке
90 градусов MITER при tau=11.25..15 даёт 8..10 faces и минимальную
площадь >=0.1368 на полевых ширинах. Ровно при переходе в SMOOTH
(tau=10/9) число faces скачет до 46..52, минимальная площадь падает
до 1.79e-6. G1 source-edge imprint occurrences = 0 на всех кадрах:
источник — SMOOTH corner arrangement, не G1. Поднимать порог
запрещено до design decision: это расширило бы диапазон дефекта.

**Решение ревьювера (по таблице `decal_c8_3_calibration.json`):
отключить SMOOTH sub-band НЕМЕДЛЕННО + чинить отдельной задачей.**
1. **C8.3-off (сейчас, однострочный диф):** SMOOTH-константа -> 0
   (sub-band выключен), углы tau <= 10 возвращаются в MITER —
   данные показывают чистый выход (8-10 faces, min area 0.137,
   стабильно по ширинам); полевые needles исчезают немедленно.
   Порог НЕ поднят (верно удержано исполнителем).
2. **C8.3-fix (отдельная задача):** диагноз фрагментации wedge-
   fill/fragment union — 46-52 faces с min area 1.79e-6 при
   ожидаемых ~2 faces на угол (бисектриса между сегментными
   quad'ами). Подозрение: почти-совпадающие кривые (бисектриса
   против crop-рёбер) в пределах кванта — болезнь «двойного
   представления» на stable-пути (принцип 13c локально, GL не
   ждать). Триаж: дамп входов arrangement одного SMOOTH-угла,
   подсчёт различных кривых в пределах 1q друг от друга.
   Acceptance повторного включения: faces на SMOOTH-угол <=
   MITER-счётчика, min area порядка сегментных faces, V течёт
   насквозь (angle-only), полевая арка рендерится цельно.


### G1.1 Needle-слайверы импринта (триаж)

На полевом скрине у гребня видны игольчатые треугольники у стыков
фейсов. Проверить: продукт G1-imprint (сплит по source-ребру
рождает вырожденные треугольники у угла/гребня) или
corner-механика. Если imprint: слайвер-merge на stable-пути по
T6-критериям (вырожденные — сливать, provenance сохранять,
видимая геометрия бенда по ребру не меняется).

Результат (`artifacts/decal_g1_1_needle_triage.json`): G1-root не
подтвердился, поэтому production merge не менялся. Exact intrinsic
G1-strip (8 граней четверть-цилиндра) на ширинах 0.74/1.27/1.61
даёт 4/8/10 faces, ноль triangle faces и минимальный угол >=89.97°.
Контрольный SMOOTH-corner при tau=10° даёт 31 face, из них 14
треугольников с минимальным углом ~4.998°. Полевые needles относятся
к C8.3 design-stop, не к G1 imprint.

### G1.2 Попарные cross-band рёбра импринта (полевой тест 3, скрин 1)

Полевой репорт: imprint создаёт вершины на обоих рейлах ленты, но
НЕ соединяет пары (верхняя_i <-> нижняя_i) рёбрами поперёк ленты —
пользователь выделил пары вручную и показал отсутствие связи.
Это обещанный функционал G1: перегиб декали ПО source-ребру
требует поперечного ребра на каждой imprint-станции. Требование:
на каждой станции импринта — ребро поперёк ленты между парными
вершинами рейлов, грань делится, бенд по source-ребру становится
геометрическим (не только вершинным). Acceptance: на полевой арке
каждая imprint-пара соединена, декаль перегибается по рёбрам
стены, shading без изломов; headless-ассерт: счётчик imprint-пар
== счётчик поперечных рёбер.

### SN-принцип: surface-native геометрия, Voronoi — мозг, не тело

Полевой вход пользователя (тест 3): старый legacy-метод работал
ТОЧНЕЕ при движении вдоль контрольных рёбер оригинального меша и
огибании его сетки (углы), а его недостатки — ровно три:
не перестраивался/замораживался при столкновении, не имел
обработки острых convex/concave углов, не умел перестраивать
направление по сетке со строительством целевых вершин/рёбер
(обволакивание сферы). UV вынесен за скобки.

Диагноз принят как архитектурный: legacy был SURFACE-NATIVE —
лента строилась по фактическим граням меша, и перегибы по рёбрам
доставались БЕСПЛАТНО, по построению. Patch Voronoi — CHART-NATIVE:
конкуренция решается в 2D, а геометрия восстанавливается обратно
на поверхность, и ВСЕ полевые дефекты V1/V2 (отсутствие
imprint-пар, V-клинья, лишние сплиты) живут именно в этом обратном
восстановлении. Маятник качнулся слишком далеко: chart стал не
только решателем, но и источником геометрии.

**Принцип (для V1/V2 и далее): геометрия ленты — surface-native,
как legacy; Voronoi/chart — слой конкуренции и параметризации.**
Разделение ролей:
- Chart/Voronoi (мозг): extents вдоль сети (где лента кончается —
  столкновения, S1-замороженные станции), классы углов по
  bands-оракулу ТОЛЬКО для in-plane поворота, admission
  (G3-measured), UV (сильная сторона chart'а — за скобками поля).
- Surface-march (тело): SEGMENT-интерьеры строятся по граням
  source-меша — каждый quad лежит в плоскости своей грани ПО
  ПОСТРОЕНИЮ, перегибы наследуют source-рёбра (G1.2-пары
  существуют автоматически, imprint как отдельный шаг отмирает
  для сегментов), объём ленты задают 1D-extents из компайла.
Слабости legacy НЕ возвращаются: freeze/столкновения — из
PV-extents; острые углы — bands-оракул; сфера — intrinsic charts
(G3/G2). Внедрение инкрементально: сначала PLANAR-бэкенд
(намеренная пересдача differential), затем developable.
C8.7 и G1.2 исполняются В ЭТОЙ конструкции, не поверх старой.

### RAIL-направление: «берега русла» (полевой дизайн-вход, тест 4)

Пользовательская модель принята как расширение SN-принципа:
фронтир Voronoi «течёт в русле», берега которого — пути рёбер
меша (rails). Два полевых случая — один механизм:
(a) стена: транс-срез ленты на junction'е скользит ВДОЛЬ
соседнего chain'а (сосед — граница, «voronoi разделяется этой
границей»); (b) сфера: лента течёт от seam-кольца к полюсу вдоль
меридианных рёбер — только так криволинейная поверхность
облегается.

Правила выбора rail — ТОЛЬКО топологические/детерминированные
(требование пользователя: никаких эвристик/подбора цифр —
пограничные случаи не должны «мигать»):
0. seam/pChain-рёбра — НИКОГДА не rail, но ГРАНИЦА: rail
   терминируется/скользит вдоль них (даёт (a) бесплатно);
1. ЕДИНСТВЕННОЕ продолжение: ребро, не делящее грань с входящим
   (edge-loop правило Blender). Продолжение существует только
   когда оно ТОПОЛОГИЧЕСКИ ЕДИНСТВЕННО (регулярная quad-вершина);
2. любая неоднозначность (валентность != 4, триангулированная
   зона: ноль или >= 2 кандидатов) — ДЕТЕРМИНИРОВАННЫЙ СТОП
   («плотина»): rail завершается станцией, событие видно в
   предпросмотре русла. Никакого углового fallback'а. НИКАКИХ
   tie-break'ов по индексам/углам/длинам;
3. мост через неоднозначность — ТОЛЬКО явная разметка ребра
   артистом (edge-mark «decal rail»); топология меша + разметка =
   полная спецификация течения. (Опционально позже: ОТДЕЛЬНЫЙ
   инструмент-ассистент, предлагающий разметку — артист
   подтверждает; ядро остаётся детерминированным);
4. терминация: полюс/N-fan (нет единственного продолжения -> CAP
   по bands-оракулу; все меридианы сходятся в вершине полюса —
   топологический факт), чужой chain (скольжение/стоп),
   alpha-reach (не эвристика — сам пользовательский параметр
   ширины).
Следствие для честности: триангулированные зоны по умолчанию —
«плотина», течение туда не идёт без разметки; отказ видимый и
предсказуемый, а не неверная догадка. Для quad-доминантных
игровых ассетов покрытие полное.
Rail-трассировка — компайл-факт (S1-safe), кэшируется в плане;
UX-идея: подсветка русла в предпросмотре до подтверждения.

Разделение ролей уточняется: rail-домен — ЕДИНСТВЕННЫЙ источник
геометрии ленты; Voronoi — конкуренция там, где встречаются
потоки (freeze/extents в rail-станциях); chart/UV строится ИЗ
rail-домена (один источник — урок GL, заложить с первого дня).
Convex-сплиты от диаграммы: предпочтительно НЕ постфактум на
готовом меше, а слияние НА ЭМИССИИ (materialization-time). Бонус
SN-геометрии: критерий слияния становится ЧИСТО СТРУКТУРНЫМ, без
epsilon — same owner + same source face (грани на surface-native
пути копланарны точно, по построению) + разделяющее ребро без
семантической нагрузки; provenance-gated, counted, не через
классовые границы; намеренные сплиты G1.2/source-фолдов не
трогать.

Статус: направление одобрено пользователем концептуально;
следующий шаг — rail-оракул отдельным документом
(docs/decal_rails.md, по образцу bands/admission оракулов) с
контрольными фикстурами ДО имплементации. Издержки/риски — в
ответе ревьювера (тест 4).

### C8.7 V-клин на crest-углах + сплиты на плоскости (полевой тест 3, скрин 2)

Полевой репорт: на углах (стык двух полей через ребро) лента
«раздвигается в стороны», образуя V-образный треугольник;
пользовательская семантика: на таких углах декаль ДВИЖЕТСЯ ВДОЛЬ
ребра угла — перегиб по общему ребру, обе стороны ленты делят
ребро сгиба, БЕЗ клина. Дизайн-направление (SN-принцип): разложение угла на dihedral
fold и in-plane turn; fold обрабатывается surface-march'ем
бесплатно (лента строится по граням — перегиб наследует ребро),
corner-компенсацию получает ТОЛЬКО in-plane составляющая; у
crest-углов она ~0 => чистый перегиб, V-клин структурно не
рождается. Плюс триаж двух
сплитов на ПЛОСКОЙ части (скрин 2, верхние): проверить на сборке
ПОСЛЕ C8.3-off — SMOOTH-остаток либо cross-backend шов.
Acceptance: сцена полевого скрина — угол без V-раскрытия, лента
непрерывна через угол, ноль лишних сплитов на плоскости.
...
merge/key-слое. Вторая волна (полевой тест 2): C8.5 -> C8.3-cal ->
G1.1 — впереди остальной очереди (пользовательски видимые дефекты
основного сценария, см. §0a).
...
**W4-данные получены; вердикт ревьювера: G3 остаётся ЗАКРЫТ до
валидации измерения.** Данные (после G1, `723607d`): dome_16x8 —
width error 0.060230 при defect 0.063974; dome_32x16 — width error
0.408860 при defect 0.016019; цилиндры 12/16/24 ~ 0. Оба купола
REJECT `DISTORTION_BUDGET_EXCEEDED`. Немонотонность (мельче сетка —
меньше дефект — В СЕМЬ РАЗ больше ошибка) нефизична: измельчение
приближает гладкую сферу, ошибка обязана монотонно убывать к
пределу гладкого случая. Данные не могут обосновывать пороги
(правило E0.b); стоп-условие W4 сработало по назначению.

Диагностический протокол (ТОЛЬКО harness/fixtures; продакшен и
admission-код не трогать):
1. **ЧТО измерялось.** Оба купола REJECT -> если harness мерил
   fallback-результат (legacy юбку), метрика бессмысленна для G3.
   Требование: E0-замер измеряет САМ intrinsic chart в
   diagnostics-режиме measure-despite-reject — вердикт admission
   записывается в отчёт, но не прерывает измерение; режим
   существует только в harness.
2. **Fixture-контракт.** Декаль на обоих куполах обязана лежать на
   ОДНОЙ широте: одинаковый polar angle, одинаковые радиус и
   абсолютная ширина. Подозрение №1: выбор кольца по ИНДЕКСУ
   вместо широты — у 32x16 кольцо того же индекса вдвое ближе к
   полюсу, радиус параллели R*sin(theta) мал, и лента у полюса
   даёт огромное ЧЕСТНОЕ искажение. Зафиксировать в отчёте
   theta/широту обеих фикстур.
3. **Определение метрики.** Записать в JSON точную формулу (какие
   точки, мировая геодезика или chart-space, max или mean) +
   ПРОФИЛЬ ошибки вдоль ленты по станциям для обоих куполов:
   константный сдвиг = масштаб/параметризация; рост к концу =
   аккумуляция; пики на швах = support path.
4. **Санити-якорь.** Цилиндры ~0 подтверждают: harness глобально
...
(width-зависимая admission и/или APPROXIMATE-tier с явным
предупреждением) — это НОВОЕ design decision, вернуться с данными,
порогов не импровизировать.

**G3-РЕШЕНИЕ ПРИНЯТО (по данным W4-fix `2bfa4bd`): ветка (a),
measured-first, БЕЗ resolution-порогов.**
Sweep монотонен и сходится: 12x6=8.447% -> 16x8=3.676% ->
20x10=1.584% -> 24x12=1.564% -> 32x16=0.919% -> 48x24=0.408% ->
64x32=0.230%; foldover/overlap=0; цилиндры на нуле. Параметризация
починена; остаток на coarse-сетках — честная дискретизационная
дисторсия полиэдральной развёртки.
1. Формулировка «включать measured-first начиная с 20x10»
   ОТКЛОНЕНА: в admission НЕТ resolution-порогов — механизм не
   знает плотности, он меряет. 12x6 и 16x8 отклоняются
   ИЗМЕРЕНИЕМ (8.45%/3.68% > 2%) — это работа механизма, а не
   исключение фикстур.
2. Имплементация по исходному плану G3: measured width error —
   первичный критерий против budget (default 0.02, существующий
   knob EP5 `decal_chart_distortion_budget`); defect-proxy
   перестаёт резать ДО измерения и остаётся ранним exit только на
   жёсткой константе (0.5 rad); бинарные гейты foldover/overlap
   без изменений.
3. Поправка acceptance G3 по честным данным: 20x10/24x12 — ADMIT
   (в бюджете); 16x8 при alpha=1/R=5 — ЧЕСТНЫЙ REJECT по
   измерению при default budget; при `budget=0.04` (knob) — ADMIT
   с визуально неразличимой ошибкой (документировать рекомендацию
   для coarse-пропсов). Ошибка ширинозависима: более узкая лента
   на том же 16x8 может честно проходить default-бюджет —
   механизм измеряет per-compile, это штатно. E.N1/E.N2 REJECT
   как прежде.
4. «Продолжать конструкцию W4» — НЕТ: сходимость достигнута,
   остаток честен; runtime-включение measured-first — и есть
   G3-имплементация.

**Разрешение ветки G3(b) (диагностика `febfa2b` валидна): дефект
параметризации, НЕ policy.** Оба подозрения ревьювера исключены
честно (одна широта theta=pi/4, R=5, alpha=1; мерился сам chart до
admission). Профиль локализует дефект в chart mapping: rail v=1.0
соответствует hinge-unfolded 0.591140 при world chord 0.591071 —
chart-координата ширины НЕ изометрична развёрнутой дистанции.
Истинная intrinsic-ошибка фикстуры (оценка тангенс-конусом,
масштаб (alpha/R)^2 = 0.04): ~0.5% при v=0.5 и ~2.4% на rail —
РЯДОМ с бюджетом, не 31-41%; немонотонность свипа — зигзаг/аспект
рёберного support-пути, зависящий от тесселяции. Policy-ветки
(width-dependent admission, APPROXIMATE-warning) ОТКЛОНЕНЫ: они
легализовали бы баг как физику. Предписание — **W4-fix
(V-изометрия ширины)**: v-координата точки = перпендикулярная
координата в развёрнутой (hinge-unfolded) плоскости
support-полосы, НЕ аккумулированная длина рёберного пути. Оракул:
`|chart_v - unfolded_dist| <= E2-допуск` на КАЖДОЙ станции —
station-profile harness (`33868c7`) и есть этот оракул, включить
его ассертом. Cyl 12/16/24 остаются ~0; re-sweep 12x6..64x32
обязан стать монотонно сходящимся к гладкому пределу (~2% на
rail при данных параметрах). Только после этого — решение G3 по
исходной развилке (ожидание: ветка (a)). Работа E-path,
