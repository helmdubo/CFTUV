...
1. **W-CP — атласный чекпойнт** (§0b): закоммитить текущее
   W1-состояние (по T7-P3.15 включительно), saddle-acceptance
   помечается известно-красным (xfail с маркером `GL-pending`),
   полный suite зелёный, EP1 байт-в-байт, короткий отчёт (SHA,
   счётчики). Дальше атлас не трогается до среза GL.
2. **W3 — C8.1 + C8.4** (stable-путь, намеренная пересдача
   эталонов): полевое расшитие у дырки. Как описано в §0b/W3.
3. **W4 — E0-замер грубых фикстур** (только данные): таблица для
   G3. Как описано в §0b/W4.
4. **C8.3 -> C8.2** (секция C8): продолжение стабильного пути;
   differential пересдаётся по каждому пункту.
5. **G1** (после C8.2, общий merge/key-слой) -> **G3** (пороги по
   данным W4) -> **G2** (border-крыло). G3+G2 закрывают полевую
   «юбку».
