...
4b. **R1.3 — RR9a: перпендикулярный снэп терминального
   ...
   при >= 2 seam-кандидатах в конечной вершине spine направляющим
   торца выбирается ближайший к каноническому перпендикуляру
   (снэп существующего направления фолбэка (c) к дискретному
   кандидату; ничья -> плотина; mark сильнее снэпа). Фикстура
   RF18. Малый срез сразу после GPU-merge; распространение на
   fold-кандидатов уровня (b) — ждёт ответа пользователя.
