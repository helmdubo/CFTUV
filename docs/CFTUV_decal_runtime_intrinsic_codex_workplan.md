...
4. Регрессия на репро design-stop: saddle, width 0.25/0.5/1.0/2.0 —
   ...
   Семнадцатый кейс — канонизация общих границ ячеек (T7-P3.13;
   chart14, isolated SEGMENT-sliver 1..5 с None/None соседями,
   region component size=1): СНАЧАЛА верификация полей по 13b
   (provenance внутренних рёбер падающего face, ширина
   None-щелей, дистанция segment-crop-end <-> kite14-side,
   representative против 12028-crop'а) — отчёт ревьюверу; при
   подтверждении гипотезы: общая граница пары смежных ячеек —
   единый компайл-объект с объединённой provenance, оба crop'а
   ссылаются; repro зелёный per-face (сепаратор виден в границе
   face), None-щели исчезают (ассерт: ноль no-owner faces шириной
   <= 1q между копиями одной идеальной границы), гейт 3.8 и
   негативы T7-P3.12 не деградируют; при неподтверждении —
   эскалация с собранными полями, кода не писать.
