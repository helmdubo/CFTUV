...
тестирование утверждал «No formal tests» при 756 живых тестах.
...
def test_large_binary_count_does_not_grow():
    ...
    85 МБ PNG-скриншотов и .blend-файлов уже лежат в истории; каждый клон
    платит за них. Правило не откатывает прошлое — оно останавливает рост.
    Новые крупные артефакты должны попадать под LFS через `.gitattributes`.
    """
