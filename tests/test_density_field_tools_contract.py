...
def test_every_perf_cap_carries_a_named_price():
    ...
    Капы подняты ценой новых СЧИТАЮЩИХСЯ доменов, а не потерей скорости —
    старая работа каждый раз оставалась прежней:

    * `building` 120 -> 180: d4 стал 111/122 против 105; семь бывших
      bridge-отказников стоят 67.6 с, старые домены не подорожали (82.1 с),
      прогон 149.71.
    * `building` 180 -> 240: цена продуктового допуска кривизны 1/80 —
      прежде отвергавшиеся на METRIC кривые домены теперь считаются и стоят
      ~+49 с на d4. Замер d4 = 198.6 при капе 180 (REJECTED); d0/d1 влезали и
      в 180 (131.7 / 125.4). Владелец ранее принял «~5 минут на `building`
      допустимо», и 240 внутри этой рамки.
    * `"2"` 17.3 -> 30.0 в гейте: d4 стал 20/32 против 19, +16.4 с, прогон
      28.30 при полевой SLA-рамке 30, которая ДЕРЖИТСЯ.
