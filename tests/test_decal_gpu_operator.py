"""F3.b интеграция GPU preview в modal `_generate` (headless).

Проверяет display-adapter контракт на уровне оператора: GPU-ветка
перехватывает ТОЛЬКО preview-кадры, ошибки evaluator'а пробрасываются
(A6-семантика modal), падение adapter'а даёт постоянный mesh fallback,
confirm (preview=False) всегда идёт exact-путём.
"""
...
import tests.test_operators  # noqa: F401 — устанавливает bpy UI-стабы
...
def test_generate_routes_preview_to_gpu_and_skips_mesh(monkeypatch):
    ...
    assert mesh_calls == []  # ноль mesh-записей во время drag
...
def test_generate_confirm_never_routes_to_gpu(monkeypatch):
    ...
    assert (
        operator._generate(_context(), _state(), "SETTINGS", preview=False)
        is sentinel
    )
...
def test_generate_adapter_failure_falls_back_to_mesh(monkeypatch):
    ...
    # Постоянный fallback: контроллер снят, handler освобождён.
