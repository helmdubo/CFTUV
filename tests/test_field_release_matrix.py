...
ДВА ВОРОТА ЗДЕСЬ КРАСНЫЕ НАМЕРЕННО. Это знамя ремонта, а не поломка теста:

* `test_wall_2_001_faces_and_coverage_are_exact` — полевой отказ владельца:
  скелет EXACT на 49 узлах, граней 0, `FACE_CHAIN_DOES_NOT_CLOSE`.
* `test_walls_012_is_exact` — вторая регрессия того же слоя, найденная
  замером: скелет обрывается на 2 узлах вместо 12,
  `SUPERLEVEL_COMPONENT_UNRESOLVABLE`.

xfail и skip им ЗАПРЕЩЕНЫ и запрет исполняется тестом
`test_red_gates_are_not_suppressed`: xfail превратил бы полевой отказ в
зелёную строку, то есть ровно в то, ради отмены чего эта карточка заведена.
Красное здесь — предъявленный факт; зелёным оно станет от ремонта ядра.
...
# ---------------------------------------------------------------------------
# 2. КРАСНОЕ ВОРОТО №1 — полевой отказ владельца на стене 2.001.
# ---------------------------------------------------------------------------


def test_wall_2_001_faces_and_coverage_are_exact():
    """Стена 2.001 обязана собрать грани и покрытие. СЕЙЧАС КРАСНОЕ.
...
# ---------------------------------------------------------------------------
# 3. КРАСНОЕ ВОРОТО №2 — walls.012. Лабораторный минимум того же слоя.
# ---------------------------------------------------------------------------


def test_walls_012_is_exact():
    """walls.012 обязан строиться целиком. СЕЙЧАС КРАСНОЕ.

    Обрыв короче и потому нагляднее, чем у 2.001: распространение
...
def test_walls_012_anchor_loci_survive():
    ...
    распространение обрывается ДО остальных десяти локусов, поэтому в якоря
    они не попали — согласия двух математик по ним нет. Сильные ворота
    walls.012 — `test_walls_012_is_exact`, и оно красное.
    """
