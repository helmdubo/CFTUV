...
ВНИМАНИЕ, срез R1d. Этот гейт требует ПОБАЙТОВОГО равенства свежего экспорта
хоста с записанным `analysis_snapshot.json`, а фикстура экспортирована под
`UNSNAPPED_EXACT_V1`, тогда как хост теперь объявляет
`SOURCE_ONLY_GRID_SNAP_V1` (`cftuv/surface_ir.py`). Значит равенства не будет,
пока владелец не переэкспортирует фикстуру в Blender: у свежего снимка другой
`snapping_law` и привязанные координаты вершин.

Это не отложенная поломка, а развилка, которую нельзя пройти обеими ногами.
Переэкспорт двигает дайджест принятого корпуса (622e1f6e…), а срез R1d обязан
был его сохранить; сохранить дайджест и переключить хост одновременно
невозможно. Выбрано сохранить дайджест: он — принятая приёмка, а расхождение
здесь видно и названо. Blender исполнителю среза недоступен, поэтому гейт не
прогонялся; замер под новым законом сделан на самой фикстуре и записан в
