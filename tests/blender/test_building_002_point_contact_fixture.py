...
РАЗВИЛКА R1d ЗАКРЫТА ПЕРЕЭКСПОРТОМ, и закрыта она ровно так, как здесь и было
предсказано. Записано это, потому что предсказание сбылось поимённо и потому что
иначе примечание читалось бы как ещё не разрешённое.

Гейт требует ПОБАЙТОВОГО равенства свежего экспорта хоста с записанным
`analysis_snapshot.json`. Прежде фикстура была экспортирована под
`UNSNAPPED_EXACT_V1`, а хост объявлял `SOURCE_ONLY_GRID_SNAP_V1`
(`cftuv/surface_ir.py`), поэтому равенства быть не могло, и выбор стоял так:
сохранить дайджест принятой приёмки (622e1f6e…) ИЛИ переключить хост. Срез R1d
выбрал сохранить дайджест и назвать расхождение.

Владелец переэкспортировал фикстуру (`9634715`), и обе половины предсказания
исполнились: отгруженный `grid_certificate` объявляет
`SOURCE_ONLY_GRID_SNAP_V1` с `source_scale = 2048`, а дайджест принятого корпуса
сдвинулся — `622e1f6e…` стал `60910ef0…`. Значит именно этот гейт снова имеет
смысл прогонять: он сверяет свежий экспорт с байтами, собранными ТЕМ ЖЕ законом,
а не с байтами прошлой политики.

Blender исполнителям срезов недоступен, поэтому гейт по-прежнему не прогонялся
ни разу. Всё, что проверяемо без него, проверено на самих байтах:
`kernel/tests/test_grid_wiring.py` (законы решётки и топология),
`kernel/tests/test_building_002_point_contact_fixture.py` (дескриптор метрики
побитово перестраивается под ОБЪЯВЛЕННЫМ законом фикстуры). Открытый счёт —
литералы `expected_kernel_results` в `tools/export_building_002_point_contact_fixture.py`
и отчёты в `artifacts/envelope_c_r2c_fixture/`: и те и другие описывают байты до
переэкспорта, и перерезать их может только тот, у кого есть Blender.
"""
